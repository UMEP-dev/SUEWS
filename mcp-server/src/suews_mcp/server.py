...
@mcp.tool(
    description="Validate a SUEWS configuration file for scientific soundness"
)
...
@mcp.tool(
    description="Get parameter suggestions based on urban context and climate"
)
...
@mcp.tool(
    description="Check compatibility between physics methods"
)
async def check_suews_physics_compatibility(
...
@mcp.tool(
    description="Generate a SUEWS configuration template"
)
...
@mcp.tool(
    description="Get detailed explanation of a SUEWS parameter"
)
...
@mcp.tool(
    description="Diagnose energy balance from SUEWS outputs"
)
...
@mcp.tool(
    description="Analyze thermal comfort from SUEWS results"
)
async def interpret_suews_thermal_comfort(
    output_path: str,
    period: str = "full"
) -> str:
    """
...
@mcp.tool(
    description="Analyze urban climate effects like heat island"
)
async def analyze_suews_urban_effects(
    output_path: str,
    rural_reference: Optional[str] = None
) -> str:
    """
...
@mcp.tool(
    description="Validate model results against observations"
)
...
@mcp.tool(
    description="Generate comprehensive insights report from SUEWS simulation"
)
