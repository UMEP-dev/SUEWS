...
class TestRSLConvergence:
    ...
    def test_rsl_convergence_high_z0_low_fai(self, base_config):
        ...
        assert not df_output['QH'].isna().any(), "QH contains NaN values - convergence failed"
        assert not df_output['QE'].isna().any(), "QE contains NaN values - convergence failed"
        assert not df_output['U10'].isna().any(), "U10 contains NaN values - convergence failed"
        assert not df_output['T2'].isna().any(), "T2 contains NaN values - convergence failed"
        ...
        assert (df_output['U10'] >= 0).all() and (df_output['U10'] < 50).all(), "U10 out of reasonable range"
        assert (df_output['T2'] > -50).all() and (df_output['T2'] < 60).all(), "T2 out of reasonable range"
    ...
    def test_rsl_neutral_stability_handling(self, base_config):
        ...
        assert (df_output['L_mod'].abs() > 100).sum() > 0, "No neutral conditions detected"
    ...
    def test_rsl_comparison_with_most(self, base_config):
        ...
        qh_mean = (df_output_rsl['QH'].abs().mean() + df_output_most['QH'].abs().mean()) / 2
        relative_diff = qh_diff / qh_mean if qh_mean > 0 else 0
        
        assert relative_diff < 2.0, f"RSL and MOST results differ too much: {relative_diff:.2f}"
    
    @pytest.mark.parametrize("fai,z0m", [
    ...
    def test_rsl_convergence_parameter_sweep(self, base_config, fai, z0m):
        ...
        assert df_output['QH'].abs().max() < 1000, f"QH unreasonable for FAI={fai}, z0m={z0m}"