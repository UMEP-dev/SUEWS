...
class RSTGenerator:
    ...
    def _get_model_children(self, model_name: str, depth: int = 0) -> dict:
        ...
        for field in model_data.get("fields", []):
            ...
            if nested_model and nested_model in self.models and nested_model not in utility_types:
                ...
                if nested_model in profile_types:
                    ...
                    if model_name == "InitialStates" and field_key in ["roofs", "walls"]:
                ...
                # This is a simple field (not a nested model)
            ...
            if level + 1 < max_level:
                ...
                if simple_fields:
                    for field in simple_fields:
                        ...
                        lines.append(f"{next_indent}{next_marker} :option:`{field_name}`")
                        lines.append("")
                ...
                if children:
                    ...
                    self._generate_hierarchy_rst_simple(children, lines, level + 1, max_level)
    ...
    def _generate_hierarchy_rst_collapsible(
        ...
        COLLAPSE_THRESHOLD = 15  # Use collapse for groups with more than this many fields
        HORIZONTAL_THRESHOLD = 8  # Use horizontal list for groups with this many or fewer fields
            ...
            if level + 1 < max_level:
                ...
                if field_count > 0:
                    if field_count > COLLAPSE_THRESHOLD:
                        ...
                        lines.append(f"{next_indent}.. collapse:: {field_count} parameters")
                        lines.append(f"{next_indent}   :class: collapse-sm")
                        lines.append("")
                        
                        # Add fields inside collapse
                        collapse_indent = next_indent + "   "
                        for field in simple_fields:
                            field_name = field['name']
                            if field_name == 'ref':
                                continue
                        ...
                        lines.append(f"{next_indent}.. hlist::")
                        lines.append(f"{next_indent}   :columns: 3")
                        lines.append("")
                        ...
                        for field in simple_fields:
                            ...
                            if field_name == 'ref':
                                continue
                            lines.append(f"{next_indent}   * :option:`{field_name}`")
                        lines.append("")
                ...
                if children:
                    self._generate_hierarchy_rst_collapsible(children, lines, level + 1, max_level)
    ...
    def _generate_hierarchy_rst_aggressive_collapse(
        ...
        COLLAPSE_THRESHOLD = 5  # Use collapse for groups with more than this many fields
        HORIZONTAL_THRESHOLD = 20  # Don't use horizontal lists, either normal or collapsed
