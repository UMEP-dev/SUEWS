...
async def example_configure_simulation():
    ...
    result = await server.call_tool(
        ...
        {
            "config_updates": {
                "site": {"name": "London", "latitude": 51.5, "longitude": -0.1},
                "surface": {
                    "fractions": {
                        "building": 0.4,
                        "paved": 0.2,
...
async def example_run_simulation():
    ...
    result = await server.call_tool(
        "run_simulation",
        {"use_sample_data": True, "save_state": True, "time_step": 3600},
    )
...
async def example_analyze_results():
    ...
    result = await server.call_tool(
        "analyze_results",
        {"results_path": "output/simulation_results.csv", "analysis_type": "summary"},
    )
...
async def example_complete_workflow():
    ...
    config_result = await server.call_tool(
        "configure_simulation",
        {
            "config_updates": {
                "site": {"name": "UrbanTestSite", "latitude": 51.5, "longitude": -0.1},
...
async def example_error_handling():
    ...
    result = await server.call_tool(
        "configure_simulation", {"config_path": "/non/existent/file.yaml"}
    )
    ...
    result = await server.call_tool(
        "analyze_results",
        {"results_path": "output/results.csv", "analysis_type": "invalid_type"},
    )
    if not result["success"]:
