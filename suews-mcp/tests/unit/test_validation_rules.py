...
class TestInputValidation:
    ...
    async def test_preprocessing_tool_validation(self, handlers):
        ...
        result = await handlers.handle_call_tool("convert_data_format", {
            "input_file": "test.csv"
    ...
    async def test_parameter_type_validation(self, handlers):
        ...
        test_cases = [
            ...
            ("validate_suews_config", {"config_file": "test.yml", "strict": "yes"}),  # Should be bool
            ("analyze_suews_output", {"output_file": "test.csv", "metrics": "QH"}),  # Should be list
        ]
...
class TestConfigurationValidation:
    ...
    def test_surface_fraction_validation(self, tmp_path):
        ...
        fraction_issues = [i for i in result.issues if "fraction" in i.get("rule", "").lower()]
    ...
    def test_invalid_surface_fractions(self, tmp_path):
        ...
        fraction_issues = [i for i in result.issues if "fraction" in i.get("message", "").lower()]
    ...
    def test_building_height_validation(self, tmp_path):
        ...
        height_issues = [i for i in result.issues if "height" in i.get("message", "").lower()]
    ...
    def test_required_fields_validation(self, tmp_path):
        ...
        missing_issues = [i for i in result.issues if "missing" in i.get("message", "").lower()]
...
class TestForcingDataValidation:
    ...
    def test_physical_range_validation(self, tmp_path):
        ...
        range_issues = [i for i in result.issues if "range" in i.get("message", "").lower() or "outside" in i.get("message", "").lower()]
        assert len(range_issues) > 0
    ...
    def test_missing_data_detection(self, tmp_path):
        ...
        missing_issues = [i for i in result.issues if "missing" in i.get("message", "").lower() or "nan" in i.get("message", "").lower()]
        assert len(missing_issues) > 0
    ...
    def test_energy_balance_validation(self, tmp_path):
        ...
        balance_issues = [i for i in result.issues if "balance" in i.get("message", "").lower() or "energy" in i.get("message", "").lower()]
...
class TestErrorHandlingPatterns:
    ...
    def test_context_aware_error_handling(self):
        ...
        context1 = ErrorContext(operation="loading configuration", file_path=Path("config.yml"))
        context2 = ErrorContext(operation="running simulation", file_path=Path("output.txt"))
    ...
    def test_error_suggestion_quality(self):
        ...
        test_cases = [
            ...
            ("Missing required column: Tair", "missing_columns", ["required", "column"]),
...
class TestToolErrorIntegration:
    ...
    async def test_invalid_json_parameter_handling(self, handlers):
        ...
        result = await handlers.handle_call_tool("run_suews_simulation", problematic_args)
