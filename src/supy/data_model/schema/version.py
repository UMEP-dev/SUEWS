...
SCHEMA_VERSIONS: dict[str, str] = {
    ...
    "2026.5.dev5": (
        ...
        "against its numeric codes at load time. `net_radiation` also "
        "accepts the orthogonal shape "
        "(`net_radiation: {scheme: narp, ldown: air}`) for the same "
        "numeric codes, and `emissions` accepts "
        "`{heat, co2.anthropogenic, co2.biogenic}` for the existing "
        "biogenic CO2 families. Canonical internal "
    ...
    "2026.5.dev7": (
        ...
        "fraction_heat_capacity_wall_external, etc. Three orthogonal moves "
    ...
    "2026.5.dev8": (
        ...
        "conductivity_wall, and fraction_heat_capacity_wall_external. The "
    ...
    "2026.5.dev11": (
        "Naming-convention completion for the STEBBS building model, "
        "combining two rename groups in a single bump (gh#1392 + gh#1394). "
        "(a) ArchetypeProperties Tier-1 completion (16 renames): archetype_* "
        "namespace prefix for whole-archetype fields (building_name -> "
        "archetype_name, building_count -> archetype_building_count, "
        "building_height -> archetype_height); geometry quantity-first reorder "
        "(footprint_area -> area_footprint, wall_external_area -> "
        "area_wall_external, internal_mass_area -> area_internal_mass) with the "
        "ratio_* category prefix for fraction fields (internal_volume_ratio -> "
        "ratio_internal_mass_volume, window_to_wall_ratio -> "
        "ratio_window_to_wall); HVAC + setpoint air_/water_ qualifier "
        ...
        "cooling_setpoint_temperature -> temperature_air_cooling_setpoint) with "
        "profile_* for the setpoint/metabolism profiles. Rename table "
        "ARCHETYPEPROPERTIES_DEV7_RENAMES; the canonical "
        "ARCHETYPEPROPERTIES_RENAMES values chain one hop further to these "
        "finals so PascalCase YAMLs still resolve in a single pass. "
        "(b) StebbsProperties Rule-2 reorder (44 renames): convection "
        "coefficients (wall_internal_convection_coefficient -> "
        "convection_coefficient_wall_internal etc., plus floor -> "
        "ground_floor), ground (external_ground_conductivity -> "
        "thermal_conductivity_ground), metabolism/occupant/daylight/lighting "
        "controls (non-physical category prefixes: threshold_metabolism, "
        "ratio_latent_sensible, control_daylight, "
        "threshold_lighting_illuminance), HVAC + setpoint air_/water_ qualifier "
        "(max_cooling_power -> power_air_cooling_max, cooling_system_cop -> "
        "efficiency_cooling_system_air, hot_water_heating_setpoint_temperature "
        "-> temperature_water_heating_setpoint), initial/climatology "
        "temperatures, and the full hot-water subsystem; profile_* for profile "
        "fields. Rename table STEBBSPROPERTIES_DEV8_RENAMES. Both tables live "
        "in src/supy/data_model/core/field_renames.py; the (2026.5.dev10 -> "
        "2026.5.dev11) migration is registered in "
        "src/supy/util/converter/yaml_upgrade.py::_HANDLERS. Bridge DataFrame "
        "columns keep the fused PascalCase ancestry via "
        "ARCHETYPEPROPERTIES_DEV7_TO_PASCAL and the chained "
        "STEBBSPROPERTIES_DEV9_TO_PASCAL map. "
        "(c) Reconciled against the Reading STEBBS team's review "
        "(D. Hertwig, 2026-05): building_type is dropped (unused by the kernel, "
        "hardcoded to 'None'; logged migration drop, not silent), and the "
        "wall/roof heat-capacity fraction moves from the outer layer to the "
