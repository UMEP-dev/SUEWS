...
class ConversionValidator:
    ...
    def validate_pre_conversion(self, input_dir: Path, from_ver: str, to_ver: str) -> Tuple[bool, List[str]]:
        ...
        for file_path in input_dir.glob("SUEWS_*.txt"):
            try:
                ...
                if df.index.duplicated().any():
                    ...
                    issues.append(f"Duplicate codes in {file_path.name}: {duplicates.tolist()}")
            except:
...
class ConversionRecovery:
    ...
    def suggest_fixes(self, error: Exception) -> List[str]:
        ...
        if "KeyError" in error_str and "Index" in error_str:
            ...
            suggestions.append("1. Check if file already has this column from previous conversion")
    ...
    def auto_fix(self, directory: Path, error: Optional[Exception] = None) -> bool:
        ...
        for filename, template in required_files.items():
            ...
            if not file_path.exists():
                try:
                    self.code_manager.templates.create_from_template(template, directory)
...
class TwoPhaseConverter:
    ...
    def convert(self, input_dir: Path, output_dir: Path, from_ver: str, to_ver: str,
                progress_callback=None) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        is_valid, issues = self.validator.validate_pre_conversion(input_dir, from_ver, to_ver)
        ...
        phase1_success = self._phase1_structure(input_dir, output_dir, from_ver, to_ver, report)
        ...
        if not phase1_success:
            if self.auto_fix:
                ...
                if self.recovery.auto_fix(output_dir):
                    ...
                    phase1_success = self._phase1_structure(input_dir, output_dir, 
                                                           from_ver, to_ver, report)
    ...
    def _phase1_structure(self, input_dir: Path, output_dir: Path, 
                         from_ver: str, to_ver: str, report: Dict) -> bool:
...
def convert_with_two_phase(input_dir: str, output_dir: str, from_ver: str, to_ver: str,
                           debug: bool = False, auto_fix: bool = True,
