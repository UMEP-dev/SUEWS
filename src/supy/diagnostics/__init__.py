...
def _load_output_dataframe(path_run_dir: Path) -> Optional[pd.DataFrame]:
...
def check_output_files_present(path_run_dir: Path) -> CheckResult:
    ...
    if list_paths:
        return CheckResult(
            ...
            severity="pass",
            passed=True,
            message="Found %d output file(s)." % len(list_paths),
...
def check_nan_proportion(path_run_dir: Path) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            severity="warning",
            passed=False,
            message="Could not read run output to compute NaN proportions: %s" % exc,
            details={"run_dir": str(path_run_dir)},
        )
    ...
    if list_offenders:
        return CheckResult(
            ...
            severity="warning",
            passed=False,
            message="NaN fraction exceeds %.0f%% in: %s"
            % (_NAN_THRESHOLD_FRACTION * 100, ", ".join(list_offenders)),
            details={
    ...
    return CheckResult(
        ...
        severity="pass",
        passed=True,
        message="NaN fractions within %.0f%% on QH/QE/QN."
        % (_NAN_THRESHOLD_FRACTION * 100),
...
def check_energy_balance_closure(path_run_dir: Path) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            severity="warning",
            passed=False,
            message="Could not read run output to compute energy balance: %s" % exc,
            details={"run_dir": str(path_run_dir)},
        )
    ...
    if not mask_valid.any():
        return CheckResult(
            ...
            details={"n_rows": int(len(df_output))},
        )
    ...
    if ratio_mean < _ENERGY_BALANCE_THRESHOLD:
        return CheckResult(
            ...
            severity="pass",
            passed=True,
            message="Mean closure residual %.3f < %.2f." % (ratio_mean, _ENERGY_BALANCE_THRESHOLD),
    ...
    return CheckResult(
        ...
        severity="warning",
        passed=False,
        message="Mean closure residual %.3f exceeds %.2f."
        % (ratio_mean, _ENERGY_BALANCE_THRESHOLD),
        details={"ratio_mean": ratio_mean, "n_rows": int(mask_valid.sum())},
    )
...
def check_run(run_dir: Path) -> List[CheckResult]:
    ...
    for check_fn in _REGISTERED_CHECKS:
        ...
        except Exception as exc:  # noqa: BLE001 - guard against buggy checks
            list_results.append(
                CheckResult(
                    name=check_fn.__name__,
                    severity="fail",
                    passed=False,
                    message="Check raised %s: %s" % (type(exc).__name__, exc),
                    details={"run_dir": str(path_run_dir)},
                )
            )
    return list_results
