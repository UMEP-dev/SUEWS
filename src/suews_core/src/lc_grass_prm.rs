...
pub fn lc_grass_prm_schema() -> Result<usize, BridgeError> {
    let mut n_flat = -1_i32;
    let mut err = -1_i32;

    unsafe {
        ffi::suews_lc_grass_prm_len(&mut n_flat as *mut i32, &mut err as *mut i32);
    }
...
pub fn lc_grass_prm_schema_info() -> Result<LcGrassPrmSchema, BridgeError> {
    let flat_len = lc_grass_prm_schema()?;
    let schema_version_runtime = lc_grass_prm_schema_version_runtime()?;
    let field_names = lc_grass_prm_field_names();

    if schema_version_runtime != LC_GRASS_PRM_SCHEMA_VERSION || flat_len != field_names.len() {
...
pub fn lc_grass_prm_schema_version() -> u32 {
    LC_GRASS_PRM_SCHEMA_VERSION
}

pub fn lc_grass_prm_schema_version_runtime() -> Result<u32, BridgeError> {
    let mut schema_version = -1_i32;
    let mut err = -1_i32;

    unsafe {
        ffi::suews_lc_grass_prm_schema_version(
...
pub fn lc_grass_prm_field_index(name: &str) -> Option<usize> {
    let names = lc_grass_prm_field_names();
    field_index(&names, name)
}

pub fn lc_grass_prm_to_map(state: &LcGrassPrm) -> BTreeMap<String, f64> {
    to_map(state)
}

pub fn lc_grass_prm_to_ordered_values(state: &LcGrassPrm) -> Vec<f64> {
    state.to_flat()
}

pub fn lc_grass_prm_from_ordered_values(values: &[f64]) -> Result<LcGrassPrm, BridgeError> {
    LcGrassPrm::from_flat(values)
}

pub fn lc_grass_prm_to_values_payload(state: &LcGrassPrm) -> LcGrassPrmValuesPayload {
    to_values_payload(state)
}

pub fn lc_grass_prm_from_values_payload(
    payload: &LcGrassPrmValuesPayload,
) -> Result<LcGrassPrm, BridgeError> {
    from_values_payload(payload)
}

pub fn lc_grass_prm_from_map(values: &BTreeMap<String, f64>) -> Result<LcGrassPrm, BridgeError> {
    let default_state = lc_grass_prm_default_from_fortran()?;
    from_map(values, &default_state)
}

pub fn lc_grass_prm_default_from_fortran() -> Result<LcGrassPrm, BridgeError> {
    let n_flat = lc_grass_prm_schema()?;
    if n_flat != LC_GRASS_PRM_FLAT_LEN {
        return Err(BridgeError::BadState);
    }
    ...
    unsafe {
        ffi::suews_lc_grass_prm_default(flat.as_mut_ptr(), n_flat as i32, &mut err as *mut i32);
    }
