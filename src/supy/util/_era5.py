...
def gen_forcing_era5(
    ...
    ----
        ...
        4. For ``start``/``end``, it is recommended to use the format ``YYYY-MM-DD`` to avoid confusion in day/month-first convensions (`an upstream known issue <https://pandas.pydata.org/docs/reference/api/pandas.to_datetime.html#pandas.to_datetime>`_ due to the ``dateutil`` behaviour)
