    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            try:
                ...
                memory_peak_mb = memory_info.peak_wss / 1024 / 1024 if hasattr(memory_info, 'peak_wss') else memory_mb
...
class TestResourceUsage:
    ...
    def large_dataset_generator(self):
        ...
        def generate_large_forcing(n_hours: int, n_variables: int = 20) -> pd.DataFrame:
            ...
            variable_names = [
                'qn', 'qh', 'qe', 'qs', 'qf', 'U', 'RH', 'Tair', 'pres', 'rain',
                'kdown', 'ldown', 'fcld', 'wuh', 'xsmd', 'lai_hp', 'lai_dp', 
                'albedo_hp', 'albedo_dp', 'z0m', 'z0v', 'porosity'
            ]
            ...
            for i, var in enumerate(variable_names[:n_variables]):
                if var in ['qn', 'qh', 'qe', 'qs']:
                    ...
                    # Temperature
                    values = 15 + 10 * np.sin(2 * np.pi * np.arange(n_hours) / 24) + np.random.normal(0, 2, n_hours)
                elif var in ['U']:
                    # Wind speed
    ...
    async def test_memory_usage_baseline(self, resource_config, resource_monitor):
        ...
        assert memory_increase < 50, f"Server initialization uses too much memory: {memory_increase:.1f} MB"
        assert peak_usage['peak_memory_mb'] < 200, f"Peak memory too high: {peak_usage['peak_memory_mb']:.1f} MB"
    ...
    async def test_memory_scaling_with_simulations(
        ...
        for n_sims in simulation_counts:
            ...
            for i in range(n_sims):
                simulation_data = {
                    ...
                    "output_data": large_dataset_generator(168, 15),  # 1 week, 15 variables
                    "metadata": {
                        "timesteps": 168,
                        "sites": 3,
            ...
            print(f"{n_sims} simulations: {memory_increase:.1f} MB increase "
        ...
        max_per_sim = max(m['memory_per_sim'] for m in memory_measurements if m['n_simulations'] > 0)
        assert max_per_sim < 10.0, f"Memory per simulation too high: {max_per_sim:.2f} MB/sim"
        
        # Memory should scale reasonably linearly
        if len(memory_measurements) >= 3:
            large_sim_mem = next(m for m in memory_measurements if m['n_simulations'] == 50)
            medium_sim_mem = next(m for m in memory_measurements if m['n_simulations'] == 10)
            
            scaling_ratio = large_sim_mem['memory_increase'] / medium_sim_mem['memory_increase']
            expected_ratio = 50 / 10  # Should be roughly 5x
            
            assert scaling_ratio < expected_ratio * 1.5, f"Memory scaling worse than linear: {scaling_ratio:.1f}x vs {expected_ratio}x expected"
    ...
    async def test_cpu_usage_patterns(self, resource_config, resource_monitor):
        ...
        operations = [
            ("initialization", lambda: handlers.handle_initialize({
                "protocol_version": "2024-11-05",
            ...
            ("concurrent_health_checks", lambda: asyncio.gather(*[
        ...
        for op_name, stats in cpu_results.items():
            if "concurrent" not in op_name:
                assert stats['peak_cpu'] < 20.0, f"{op_name} CPU usage too high: {stats['peak_cpu']:.1f}%"
            else:
                # Concurrent operations can use more CPU
                assert stats['peak_cpu'] < 50.0, f"{op_name} CPU usage too high: {stats['peak_cpu']:.1f}%"
    ...
    async def test_file_handle_usage(self, resource_config, resource_monitor, tmp_path):
        ...
        for i in range(10):
            config_args = {"config_file": str(temp_files[i % len(temp_files)]), "strict": False}
            ...
            with patch.object(handlers, '_call_validation_tool') as mock_validate:
                ...
                result = await handlers.handle_call_tool("validate_suews_config", config_args)
        ...
        if peak_usage.get('peak_fds', 0) > 0:
            assert peak_usage['peak_fds'] < initial_fds + 50, f"Peak file handle usage too high: {peak_usage['peak_fds']}"
    ...
    async def test_thread_usage_patterns(self, resource_config, resource_monitor):
        ...
        if peak_usage.get('peak_threads', 0) > 0:
            ...
            assert peak_usage['peak_threads'] < expected_max_threads, \
    ...
    async def test_long_running_resource_stability(
        ...
        for i in range(5):
            handlers._active_simulations[f"long_running_sim_{i}"] = {
                ...
                "output_data": large_dataset_generator(8760, 10),  # 1 year, 10 variables
                "metadata": {
                    "timesteps": 8760,
        ...
        while time.time() - start_time < operation_duration:
            ...
            if operation_count % 10 == 0:
                ...
                with patch.object(handlers, '_call_validation_tool') as mock_validate:
                    mock_validate.return_value = {
                        "content": [{"type": "text", "text": "Stability test validation"}],
                        "is_error": False
                    }
                    operations.append(handlers.handle_call_tool(
        ...
        print(f"  Memory - Min: {memory_stats.get('min', 0):.1f} MB, "
              f"Max: {memory_stats.get('max', 0):.1f} MB, "
              f"Avg: {memory_stats.get('mean', 0):.1f} MB")
        print(f"  CPU - Min: {cpu_stats.get('min', 0):.1f}%, "
              f"Max: {cpu_stats.get('max', 0):.1f}%, "
              f"Avg: {cpu_stats.get('mean', 0):.1f}%")
        
        # Stability assertions
        memory_range = memory_stats.get('max', 0) - memory_stats.get('min', 0)
        assert memory_range < 100, f"Memory usage too variable: {memory_range:.1f} MB range"
        
        # CPU should not be consistently high
        assert cpu_stats.get('mean', 0) < 15.0, f"Average CPU too high: {cpu_stats.get('mean', 0):.1f}%"
        
        # Should have completed many operations
        operations_per_second = operation_count / monitoring_duration
        assert operations_per_second > 1.0, f"Operation rate too low: {operations_per_second:.2f} ops/sec"
    ...
    def test_resource_monitoring_overhead(self, resource_config):
        ...
        overhead_ratio = with_monitor_time / no_monitor_time if no_monitor_time > 0 else 1
        ...
        assert overhead_ratio < 1.1, f"Resource monitoring overhead too high: {overhead_ratio:.2f}x"
