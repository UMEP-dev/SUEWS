...
use module_type_landcover, only: LC_BSOIL_PRM
...
subroutine lc_bsoil_prm_unpack(flat, n_flat, state, err)
   implicit none

   real(c_double), intent(in) :: flat(*)
   integer(c_int), intent(in) :: n_flat
   type(LC_BSOIL_PRM), intent(out) :: state
   integer(c_int), intent(out) :: err
...
end subroutine lc_bsoil_prm_unpack
