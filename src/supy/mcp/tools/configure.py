...
class ConfigureSimulationTool(MCPTool):
    ...
    async def _execute(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validate_only = (
            self._validate_parameter(arguments, "validate_only", bool, required=False)
            or False
        )
        site_name = self._validate_parameter(
            arguments, "site_name", str, required=False
        )
        ...
        try:
            ...
            if save_path and validation_result["valid"]:
                save_result = self._save_configuration(
                    config_obj, save_path, save_format
                )
            ...
            return self.translator.create_structured_response(
                success=False, errors=[f"Configuration loading failed: {str(e)}"]
            )
    ...
    def _save_configuration(
        self, config_obj, save_path: str, save_format: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        save_info = {"saved": False, "path": None, "format": save_format}
    ...
    def _get_config_summary(self, config_obj) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(config_obj, "surface") and hasattr(
                config_obj.surface, "fractions"
            ):
        ...
        try:
            ...
            try:
                ...
                validation_info["warnings"].append(
                    f"Advanced validation warning: {str(e)}"
                )
