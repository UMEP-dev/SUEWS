...
class SurfaceParametersStep(WizardStep):
    ...
    def collect_input(self) -> Dict[str, Any]:
        ...
        console.print("[dim]Enter the fraction of each surface type (must sum to 1.0)[/dim]\n")
        ...
        for surface, description in self.surface_types.items():
            ...
            if surface == "water" and total < 1.0:
                ...
                console.print(f"[dim]Suggested value for {surface}: {suggested} (to sum to 1.0)[/dim]")
                default = suggested
            
            value = FloatPrompt.ask(
                f"{surface.ljust(15)} ({description})",
                default=default
            )
        ...
        configure_props = Confirm.ask(
            "Configure surface properties (albedo, emissivity)?",
            default=False
        )
            ...
            for surface in self.surface_types:
                if fractions.get(surface, 0) > 0:
                    default_alb = self.session.get_value(
                        f"surface.albedo.{surface}",
                        self._get_default_albedo(surface)
                    )
                    albedos[surface] = FloatPrompt.ask(
                        f"Albedo for {surface}",
                        default=default_alb
                    )
            ...
            for surface in self.surface_types:
                if fractions.get(surface, 0) > 0:
                    ...
                    emissivities[surface] = FloatPrompt.ask(
                        f"Emissivity for {surface}",
                        default=default_emis
                    )
        ...
        configure_morph = Confirm.ask(
            "Configure morphology parameters?",
            default=True
        )
    ...
    def validate(self, data: Dict[str, Any]) -> bool:
        ...
        for surface, fraction in fractions.items():
            if fraction < 0:
                self.session.add_validation_error(
                    f"fractions.{surface}",
                    f"Fraction cannot be negative ({fraction})"
                )
                valid = False
            elif fraction > 1:
                self.session.add_validation_error(
                    f"fractions.{surface}",
                    f"Fraction cannot exceed 1.0 ({fraction})"
                )
                valid = False
        ...
        for surface in self.surface_types:
            if surface in fractions and fractions[surface] > 0:
                table.add_row(
                    ...
                    f"{emissivities.get(surface, '-'):.2f}" if surface in emissivities else "-"
                )
