...
def _load_run_dataframe(path_run_dir: Path) -> pd.DataFrame:
    ...
    if not list_paths:
        raise FileNotFoundError(
            "No df_output*.parquet / *.csv / *_SUEWS_*.txt under %s" % path_run_dir
        )
...
def _build_text_message(data: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "Period: %s -> %s (%d steps)"
        % (period.get("start"), period.get("end"), data["n_steps"]),
        "",
        "Variable summary:",
        "  %-20s %12s %12s %12s %8s" % ("name", "mean", "min", "max", "nan_pct"),
    ]
    for var in data["variables"]:
        lines.append(
            "  %-20s %12s %12s %12s %7.1f%%"
            % (
                var["name"],
                "%.4g" % var["mean"] if var["mean"] is not None else "n/a",
                "%.4g" % var["min"] if var["min"] is not None else "n/a",
                "%.4g" % var["max"] if var["max"] is not None else "n/a",
                var["nan_pct"],
            )
        )
    if data.get("notes"):
...
def summarise_output_cmd(run_dir: str, variables: str, output_format: str) -> None:
    ...
    try:
        ...
        message = "Failed to load run output: %s" % exc
        if json_mode:
            Envelope.error(
                errors=[message],
                command=command,
    ...
    if list_variables:
        ...
        for missing in list_missing:
            list_notes.append("variable %r not found in output" % missing)
