...
def version(ctx, files, update, target_version, backup):
    ...
    if not quiet:
        if update:
            console.print(f"[bold blue]Updating schema versions to {target_version or CURRENT_SCHEMA_VERSION}[/bold blue]\n")
        else:
        ...
        try:
            ...
            if update:
                ...
                if current_version != new_version:
                    # Backup if requested
                    if backup:
                        backup_path = path.with_suffix(f'.backup-{datetime.now():%Y%m%d-%H%M%S}.yml')
...
@click.option('--output-dir', '-o', type=click.Path(), help='Output directory for migrated files')
    ...
    for file_path in track(files, description="Migrating...", disable=quiet):
        ...
        try:
            ...
            if current_version == target:
                results.append((path.name, "Already at target version", "[yellow]Skipped[/yellow]"))
                continue
            
            # Perform migration
            if not dry_run:
                migrated = migrator.migrate(config, from_version=current_version, to_version=target)
                ...
                is_valid, _ = validate_file_against_schema(out_file, schema_version=target, show_details=False)
                
                if is_valid:
                    results.append((path.name, f"{current_version} → {target}", "[green]✓ Success[/green]"))
                else:
                    results.append((path.name, f"{current_version} → {target}", "[yellow]⚠ Needs review[/yellow]"))
            else:
                results.append((path.name, f"{current_version} → {target}", "[dim]Would migrate[/dim]"))
...
@click.option('--strict', '-s', is_flag=True, help='Exit with error on validation failure')
...
def validate(ctx, files, schema_version, strict, format):
    ...
    for file_path in files:
        path = Path(file_path)
        is_valid, errors = validate_file_against_schema(path, schema_version=schema_version, show_details=verbose)
...
def info(ctx):
    ...
    console.print("  • Migrate: [cyan]suews-schema migrate old.yml --target-version 2.0[/cyan]")
    console.print("  • Export schema: [cyan]suews-schema export -o schema.json[/cyan]")
    
    console.print("\n[bold]Integration:[/bold]")
    console.print("  • Use with CI/CD: [cyan]suews-schema validate *.yml --strict[/cyan]")
    console.print("  • Batch operations: [cyan]suews-schema version configs/*.yml --update[/cyan]")
