...
pub fn lc_bsoil_prm_schema() -> Result<usize, BridgeError> {
    let mut n_flat = -1_i32;
    let mut err = -1_i32;

    unsafe {
        ffi::suews_lc_bsoil_prm_len(&mut n_flat as *mut i32, &mut err as *mut i32);
    }
...
pub fn lc_bsoil_prm_schema_info() -> Result<LcBsoilPrmSchema, BridgeError> {
    let flat_len = lc_bsoil_prm_schema()?;
    let schema_version_runtime = lc_bsoil_prm_schema_version_runtime()?;
    let field_names = lc_bsoil_prm_field_names();

    if schema_version_runtime != LC_BSOIL_PRM_SCHEMA_VERSION || flat_len != field_names.len() {
...
pub fn lc_bsoil_prm_schema_version() -> u32 {
    LC_BSOIL_PRM_SCHEMA_VERSION
}

pub fn lc_bsoil_prm_schema_version_runtime() -> Result<u32, BridgeError> {
    let mut schema_version = -1_i32;
    let mut err = -1_i32;

    unsafe {
        ffi::suews_lc_bsoil_prm_schema_version(
...
pub fn lc_bsoil_prm_field_index(name: &str) -> Option<usize> {
    let names = lc_bsoil_prm_field_names();
    field_index(&names, name)
}

pub fn lc_bsoil_prm_to_map(state: &LcBsoilPrm) -> BTreeMap<String, f64> {
    to_map(state)
}

pub fn lc_bsoil_prm_to_ordered_values(state: &LcBsoilPrm) -> Vec<f64> {
    state.to_flat()
}

pub fn lc_bsoil_prm_from_ordered_values(values: &[f64]) -> Result<LcBsoilPrm, BridgeError> {
    LcBsoilPrm::from_flat(values)
}

pub fn lc_bsoil_prm_to_values_payload(state: &LcBsoilPrm) -> LcBsoilPrmValuesPayload {
    to_values_payload(state)
}

pub fn lc_bsoil_prm_from_values_payload(
    payload: &LcBsoilPrmValuesPayload,
) -> Result<LcBsoilPrm, BridgeError> {
    from_values_payload(payload)
}

pub fn lc_bsoil_prm_from_map(values: &BTreeMap<String, f64>) -> Result<LcBsoilPrm, BridgeError> {
    let default_state = lc_bsoil_prm_default_from_fortran()?;
    from_map(values, &default_state)
}

pub fn lc_bsoil_prm_default_from_fortran() -> Result<LcBsoilPrm, BridgeError> {
    let n_flat = lc_bsoil_prm_schema()?;
    if n_flat != LC_BSOIL_PRM_FLAT_LEN {
        return Err(BridgeError::BadState);
    }
    ...
    unsafe {
        ffi::suews_lc_bsoil_prm_default(flat.as_mut_ptr(), n_flat as i32, &mut err as *mut i32);
    }
