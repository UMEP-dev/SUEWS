...
class AdvancedOptionsStep(WizardStep):
    ...
    def collect_input(self) -> Dict[str, Any]:
        ...
        use_defaults = Confirm.ask(
            "\nWould you like to use recommended default physics options?",
            default=True
        )
        ...
        if advanced_options.get('storageheatmethod') == 1:  # OHM selected
            advanced_options['ohmincqf'] = 1 if Confirm.ask(
                "Include anthropogenic heat (QF) in OHM calculations?",
                default=True
            ) else 0
        else:
        ...
        # RSL settings
        use_rsl = Confirm.ask(
            "\nEnable roughness sublayer (RSL) calculations?",
            default=False
        )
        ...
        use_stebbs = Confirm.ask(
            "\nEnable STEBBS building energy model?",
            default=False
        )
    ...
    def validate(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if data.get('snowuse', 0) == 1 and data.get('waterusemethod', 0) == 2:
                console.print("[yellow]Warning: Snow model with automatic irrigation may conflict[/yellow]")
            ...
            test_config = {
                'model': {
                    'physics': {k: {'value': v} for k, v in data.items()}
                }
            }
            ...
            if not is_valid:
                console.print("[yellow]Note: Some options may require additional configuration[/yellow]")
