...
class TestCompleteDataWorkflows:
    ...
    def raw_meteorological_data(self):
        ...
        solar_base = np.maximum(
            0, 800 * np.sin(solar_angle) * np.maximum(0, np.cos(solar_angle))
        )
        ...
        rel_humidity = np.clip(
            80 - (air_temperature - 20) * 2 + np.random.normal(0, 8, n_hours), 30, 95
        )
        ...
        precipitation = np.where(
            rain_prob < 0.95, 0, np.random.exponential(2.0, n_hours)
        )
            ...
            "csv_format": pd.DataFrame(
                {
                    ...
                    "year": [2024] * n_hours,
                    "day_of_year": [
                        ((base_date + pd.Timedelta(hours=h)).dayofyear) for h in hours
                    ],
        ...
        problematic_data = pd.DataFrame(
            {
                ...
                "imin": [0] * n_hours,
                "qn": [-999] * 5
                + list(np.random.normal(100, 30, n_hours - 5)),  # Missing values
                "qh": list(np.random.normal(50, 20, n_hours)),
                "qe": list(np.random.normal(30, 15, n_hours)),
                "qs": list(np.random.normal(10, 10, n_hours)),
                "qf": [-50]
                + list(np.random.uniform(10, 30, n_hours - 1)),  # Invalid negative
                "U": [-2.0]
                + list(np.random.uniform(1, 8, n_hours - 1)),  # Invalid negative wind
                "RH": [150]
                + list(np.random.uniform(40, 85, n_hours - 1)),  # Out of range humidity
                "Tair": [-100]
                + list(np.random.normal(18, 4, n_hours - 1)),  # Invalid temperature
                "pres": [50]
                + list(np.random.normal(101.3, 2, n_hours - 1)),  # Invalid pressure
                "rain": list(np.random.exponential(0.1, n_hours)),
                "kdown": [-100]
                + list(
                    np.maximum(0, np.random.normal(300, 100, n_hours - 1))
                ),  # Negative solar
            }
        )
    ...
    async def test_multi_format_conversion_workflow(
        self, raw_meteorological_data, tmp_path
    ):
    ...
    async def test_error_recovery_workflow(self, tmp_path):
        ...
        result = validator.validate_config(
            config_path=config_file, strict_mode=True, check_file_paths=True
        )
        ...
        assert any(
            word in combined_messages
            for word in ["invalid", "missing", "range", "file"]
        )
...
class TestWorkflowIntegration:
    ...
    async def test_preprocessing_to_simulation_workflow(
        ...
        forcing_data = pd.DataFrame(
            {
                ...
                "qn": np.maximum(
                    0,
                    400 * np.sin(np.pi * np.arange(24) / 24)
                    + np.random.normal(0, 30, 24),
                ),
                "qh": np.maximum(
                    0,
                    200 * np.sin(np.pi * np.arange(24) / 24)
                    + np.random.normal(0, 20, 24),
                ),
                "qe": np.maximum(
                    0,
                    150 * np.sin(np.pi * np.arange(24) / 24)
