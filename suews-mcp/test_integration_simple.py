...
async def test_basic_functionality():
    ...
    try:
        ...
        print(f"  - Tools enabled: simulation={config.enable_simulation_tool}, validation={config.enable_validation_tool}, analysis={config.enable_analysis_tool}")
    ...
    try:
        ...
        print(f"  - Active simulations tracking: {len(handlers._active_simulations)} simulations")
            ...
            try:
                if handlers._configure_tool:
                    ...
                    if config_result.get("success"):
                        ...
                        print(f"    - Configure tool failed (expected): {config_result.get('errors', [])[:1]}")
            except Exception as e:
                print(f"  - Configure tool test failed (expected in test env): {str(e)[:60]}...")
        else:
    ...
    try:
        ...
        list_result = await handlers.handle_call_tool("list_resources", {"resource_type": "all"})
        print("✓ Resource listing works")
        
        # Test security - try path traversal
        try:
            security_result = await handlers.handle_call_tool("get_resource", {
...
def test_benchmark_data_availability():
    ...
    config_path = Path(__file__).parent.parent / "test" / "fixtures" / "benchmark1" / "benchmark1.yml"
