...
class ErrorPatternMatcher:
    ...
    def _initialize_patterns(self) -> List[Tuple[re.Pattern, Dict[str, Any]]]:
        ...
        return [
            ...
            (re.compile(r"(FileNotFoundError|No such file|cannot find).*?([\w/\\.-]+)", re.IGNORECASE), {
            ...
            (re.compile(r"surface.*fraction.*sum.*not.*equal.*1|fraction.*validation.*failed", re.IGNORECASE), {
            ...
            (re.compile(r"missing.*required.*column|column.*not.*found|KeyError.*['\"]([\w_]+)['\"]", re.IGNORECASE), {
            ...
            # Time series gaps
            (re.compile(r"time.*gap|missing.*data.*points|discontinuous.*time.*series", re.IGNORECASE), {
                "category": ErrorCategory.DATA_QUALITY,
            ...
            (re.compile(r"negative.*(radiation|pressure|wind|humidity)|invalid.*physical.*value", re.IGNORECASE), {
                "category": ErrorCategory.DATA_QUALITY,
                "severity": ErrorSeverity.ERROR,
            ...
            # Memory errors
            (re.compile(r"MemoryError|out of memory|cannot allocate|memory allocation failed", re.IGNORECASE), {
            ...
            # Stability issues
            (re.compile(r"numerical.*instability|solution.*diverged|NaN.*detected|overflow", re.IGNORECASE), {
            ...
            (re.compile(r"unsupported.*python.*version|requires.*python|python.*compatibility", re.IGNORECASE), {
            ...
            # SuPy import errors
            (re.compile(r"ImportError.*supy|No module.*supy|cannot import.*supy", re.IGNORECASE), {
                "category": ErrorCategory.DEPENDENCY,
                "severity": ErrorSeverity.CRITICAL,
            ...
            (re.compile(r"version.*mismatch|incompatible.*version|outdated.*configuration", re.IGNORECASE), {
                "category": ErrorCategory.CONFIGURATION,
                "severity": ErrorSeverity.ERROR,
...
class SUEWSErrorHandler:
    ...
    def diagnose(self, 
                 error: Exception,
                 context: ErrorContext,
                 include_traceback: bool = False) -> DiagnosedError:
    ...
    def _create_from_pattern(self,
                             error: Exception,
                             context: ErrorContext,
                             pattern_info: Dict) -> DiagnosedError:
    ...
    def _create_generic_diagnosis(self,
                                  error: Exception,
                                  context: ErrorContext) -> DiagnosedError:
    ...
    def _find_related_errors(self, error: DiagnosedError) -> List[str]:
        ...
        for historical in self.error_history[-10:]:  # Check last 10 errors
            if historical == error:
                continue
            if (historical.category == error.category and 
                historical.root_cause != error.root_cause):
