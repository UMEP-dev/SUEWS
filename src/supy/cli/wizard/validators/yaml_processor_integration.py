        ...
        try:
            ...
            if Path(report_file).exists():
                ...
                if "critical missing parameter(s)" in report_content:
                    messages.append("⚠️ Critical parameters missing - these must be provided")
                    
                if "optional missing parameter(s)" in report_content:
                    messages.append("ℹ️ Optional parameters missing - defaults will be used")
                    
                if "renamed parameter(s)" in report_content:
                    messages.append("🔄 Some parameters have been renamed to match current version")
    ...
    def get_quick_validation(
        ...
        if field_name.startswith('fr_') and context:
            ...
            if abs(total - 1.0) > 0.01:  # Allow small tolerance
                return False, f"Surface fractions must sum to 1.0 (current: {total:.2f})"
    ...
    def suggest_fixes(self, error_messages: List[str]) -> List[str]:
        ...
        for error in error_messages:
            if "surface fractions" in error.lower():
                suggestions.append("💡 Ensure all surface fractions (paved, buildings, vegetation, etc.) sum to exactly 1.0")
            elif "missing parameter" in error.lower():
                suggestions.append("💡 Use the wizard to fill in all required parameters")
            elif "renamed parameter" in error.lower():
                suggestions.append("💡 Parameter names have been automatically updated to match the current version")
            elif "physics option" in error.lower():
                suggestions.append("💡 Check that selected physics options are compatible with each other")
            elif "timezone" in error.lower():
                suggestions.append("💡 Timezone should be an offset from UTC (e.g., -6 for CST, 0 for GMT)")
