...
class TestConfigureSimulationTool:
    ...
    def test_get_parameters(self, tool):
        ...
        assert len(params) >= 4  # At least config_path, config_updates, validate_only, site_name
...
class TestRunSimulationTool:
    ...
    def test_get_parameters(self, tool):
        ...
        params = tool.get_parameters()
        assert isinstance(params, list)
        assert len(params) >= 6  # forcing_path, config_path, use_sample_data, start_time, end_time, etc.
        ...
        with patch('src.supy.mcp.tools.run.load_SampleData') as mock_load:
            with patch('src.supy.mcp.tools.run.run_supy') as mock_run:
                ...
                mock_load.return_value = (
                    ...
                    pd.DataFrame({'T2': [15]}, index=pd.date_range('2024-01-01', periods=1, freq='h'))  # df_forcing
                )
                
                # Mock simulation run
                mock_run.return_value = (
                    pd.DataFrame({'QH': [100]}, index=pd.date_range('2024-01-01', periods=1, freq='h')),
                    pd.DataFrame({'var': [2]})
                )
...
class TestAnalyzeResultsTool:
    ...
    def test_get_parameters(self, tool):
        ...
        params = tool.get_parameters()
        assert isinstance(params, list)
        assert len(params) >= 5  # results_path, analysis_type, variables, time_period, etc.
...
class TestParameterTranslator:
    ...
    def test_validate_file_path(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.txt') as f:
            with pytest.raises(ValueError):
                ParameterTranslator.validate_file_path(f.name, extensions=['.yaml', '.yml'])
    ...
    def test_serialize_dataframe(self):
        ...
        # Test with stats
        result_with_stats = ParameterTranslator.serialize_dataframe(df, include_stats=True)
