...
class SUEWSBridge:
    ...
    def validate_config(
        ...
        try:
            if use_conditional:
                ...
                config = SUEWSConfig.model_validate(config_dict, context={"conditional_validation": True})
            else:
                # Full validation
    ...
    def get_required_parameters(
        self, physics_methods: Dict[str, Any]
    ) -> Dict[str, List[str]]:
        """
        ...
        if controller.radiation_spartacus_enabled:
            requirements["SPARTACUS"] = [
                "height_mean", "height_stdev", "frontal_area_index",
                "plan_area_fraction", "wall_specular_fraction"
            ]
        
        if controller.storage_estm_enabled:
            requirements["ESTM"] = [
                "estm_code", "Internal_thickness_*", "Internal_k_*",
                "Internal_volumetric_heat_capacity_*", "thickness_*_*",
                "k_*_*", "volumetric_heat_capacity_*_*"
            ]
        
        if controller.diagmethod_rst_enabled:
            requirements["RST"] = [
                "height_mean", "height_stdev", "z",
                "displacement_height", "frontal_area_index"
            ]
    ...
    def _get_basic_requirements(self, physics_methods: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if physics_methods.get("StorageHeatMethod") == "ESTM":
            requirements["ESTM"] = [
                "estm_code", "thermal layer parameters"
            ]
    ...
    def check_method_compatibility(
        ...
        if methods.get("StorageHeatMethod") == "OHM" and methods.get("EmissionsMethod") == "PreprocessedWrfspecial":
            warnings.append({
