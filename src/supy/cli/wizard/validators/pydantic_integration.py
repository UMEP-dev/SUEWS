...
class PydanticValidator:
    ...
    def validate_complete_config(self, config_dict: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
    ...
    def _structure_config(self, wizard_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config = {
            ...
            "description": wizard_config.get("site", {}).get("description", "Generated by SUEWS Wizard"),
            "model": {
                "control": {},
                "physics": {}
            },
        ...
        if "simulation" in wizard_config:
            ...
            if "start_date" in sim:
                config["model"]["control"]["start_time"] = {"value": sim["start_date"] + " 00:00"}
                
            if "end_date" in sim:
                config["model"]["control"]["end_time"] = {"value": sim["end_date"] + " 23:00"}
        
        # Map forcing data
        if "forcing" in wizard_config:
            forcing = wizard_config["forcing"]
            if "file_path" in forcing:
                config["model"]["control"]["forcing_file"] = {"value": forcing["file_path"]}
        
        # Set default physics methods if not specified
        config["model"]["physics"] = {
            "netradiationmethod": {"value": 3},  # NARP
            "emissionsmethod": {"value": 2},     # Järvi et al. 2011
    ...
    def _add_default_surface_properties(
        ...
        # Emissivity
        if "emissivity" in surface_props and surface_type in surface_props["emissivity"]:
            ...
            surface_dict["emis"] = {"value": default_emissivities.get(surface_type, 0.95)}
