...
as a floating-point number — ``2026.6.dev2`` comes after ``2026.5``,
...
**Schema 2026.6.dev2** (current; development)
   Development schema for forcing-clock declaration work. It adds
   ``model.control.forcing.timestamp_reference`` with default
   ``local_standard_time`` and accepted values ``local_standard_time`` and
   ``utc``. Existing YAMLs remain valid because omitted values preserve the
   previous local-standard-time forcing convention.
...
**Schema 2026.6.dev1** (development)
   ...
   ``(2026.5 -> current 2026.6 development)`` handler is a no-op content migration
...
**Schema 2026.5** (released with 2026.6.5)
   ...
   ``src/supy/util/converter/yaml_upgrade.py::_HANDLERS`` applies the
   union of every dev-cycle delta, logging each rename and drop. The
   union of breaking changes against ``2026.4`` is:

   - **Fused field names to snake_case** (#1256, Category 1): for
     example ``netradiationmethod`` -> ``net_radiation_method``,
     ``soildepth`` -> ``soil_depth``, ``baset`` ->
     ``base_temperature``, ``crwmax`` ->
     ``water_holding_capacity_max``, and 55 more across
     ``ModelPhysics``, ``SurfaceProperties``, ``LAIParams``,
     ``VegetatedSurfaceProperties``, ``EvetrProperties``,
     ``DectrProperties`` and ``SnowParams``.
   ...
   - **STEBBS YAML fully snake_case** (#1334, #1337, #1327): the
     ...
     ``building_archetype``, ``stebbs`` and ``snow``), and the
     hot-water subsystem is unified under ``hot_water_*`` (the opaque
     ``dhw_*`` prefix is dropped).
   - **Naming-convention reorder** (#1392, #1394, #1452): field names
     are reordered quantity-first
     (``wall_external_thickness`` -> ``thickness_wall_outer``), the
     ``archetype_*`` namespace prefix is applied to whole-archetype
     fields, and ``building_type`` is dropped.
   - **``model.control`` restructure** (#1372, #1420):
     ``forcing_file`` -> ``forcing.file`` and ``output_file`` ->
     ``output`` (inner ``path`` -> ``dir``; the legacy string form is
     removed). Per-land-cover ``lai_<surface>`` / ``wuh_<surface>``
     forcing columns are now accepted.
   - **STEBBS physics nested** (#1456): the flat ``model.physics``
     STEBBS switches fold under ``model.physics.stebbs``
     (``stebbs.enabled`` + ``stebbs.parameters``; ``capacitance``,
     ``setpoint``, ``same_albedo_*`` and ``same_emissivity_*`` move at
     their leaf names).
   - **``frontal_area_index`` selector** (#1495): reduced to
     ``observed`` / ``modelled``; the ``provided``, ``use_provided``
     and ``simple_scheme`` aliases are retired.

   Legacy YAMLs continue to load through the Pydantic backward-compat
   shims with a ``DeprecationWarning``; bridge DataFrame / Fortran
   column names are unchanged, so there is no model-output change
   versus ``2026.4`` (shipped with 2026.4.3).
