...
class AnalyzeResultsTool(MCPTool):
    ...
    async def _execute(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results_path = self._validate_parameter(
            arguments, "results_path", str, required=True
        )
        ...
        try:
            ...
            if variables:
                ...
                missing_vars = [
                    var for var in var_list if var not in df_results.columns
                ]
                ...
                if not available_vars:
                    return self.translator.create_structured_response(
                        success=False,
                        errors=[
                            "None of the requested variables are available in the results"
                        ],
                    )
            ...
            if analysis_type == "summary":
                ...
                analysis_results = self._analyze_temporal_patterns(
            ...
            if output_format == "summary":
                ...
                analysis_info["summary"] = self._create_text_summary(
            ...
            return self.translator.create_structured_response(
                success=False, errors=[f"Analysis failed: {str(e)}"]
            )
    ...
    def _analyze_energy_balance(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for var in available_vars:
            ...
            energy_analysis["energy_statistics"][var] = {
                ...
                "total_mj_m2": float(
                    series.sum() * 3600 / 1e6
                ),  # Convert W m-2 * hours to MJ m-2
            }

        # Daily aggregations
        df_daily = df[available_vars].resample("D").mean()
        energy_analysis["daily_totals"] = self.translator.serialize_dataframe(
            df_daily, max_rows=10
        )

        # Monthly aggregations
        if len(df) > 30:  # Only if we have enough data
            df_monthly = df[available_vars].resample("M").mean()
            energy_analysis["monthly_means"] = self.translator.serialize_dataframe(
                df_monthly, max_rows=12
            )

        # Energy balance closure if QN is available
        if "QN" in available_vars:
            energy_balance_vars = [
                var for var in ["QH", "QE", "QS", "QF"] if var in available_vars
            ]
    ...
    def _analyze_water_balance(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        water_analysis = {"available_variables": available_vars, "water_statistics": {}}
    ...
    def _analyze_temporal_patterns(
        self, df: pd.DataFrame, period: Optional[str]
    ) -> Dict[str, Any]:
        """Analyze temporal patterns in the data."""
        temporal_analysis = {"period": period or "hourly", "patterns": {}}

        # Diurnal patterns (hourly averages)
        hourly_means = df.groupby(df.index.hour).mean()
        temporal_analysis["patterns"]["diurnal"] = self.translator.serialize_dataframe(
            hourly_means
        )
        ...
        if len(df) > 365 * 24:  # More than a year of hourly data
            ...
            temporal_analysis["patterns"]["seasonal"] = (
                self.translator.serialize_dataframe(seasonal_means)
            )

        # Weekly patterns
        weekly_means = df.groupby(df.index.dayofweek).mean()
        temporal_analysis["patterns"]["weekly"] = self.translator.serialize_dataframe(
            weekly_means
        )
        ...
        if period and resample_output:
            try:
                ...
                if period in period_map:
                    ...
                    temporal_analysis["resampled_data"] = (
                        self.translator.serialize_dataframe(df_resampled)
                    )
            except Exception:
    ...
    def _perform_comparison(
        self, df_results: pd.DataFrame, comparison_path: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if df_reference is None:
                return {
                    "error": f"Failed to load comparison data from {comparison_path}"
                }
            ...
            if not common_vars:
                return {
                    "error": "No common variables found between results and reference data"
                }
            ...
            if len(common_index) == 0:
                return {
                    "error": "No common time period found between results and reference data"
                }
    ...
    def _create_text_summary(
        self, analysis_results: Dict[str, Any], df: pd.DataFrame
    ) -> str:
        ...
        if "variable_summary" in analysis_results:
            ...
            for var, stats in list(analysis_results["variable_summary"].items())[:5]:
                summary_lines.append(
                    f"  {var}: mean={stats['mean']:.2f}, std={stats['std']:.2f}"
                )
