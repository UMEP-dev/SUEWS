    ...
    if path.is_dir():
        ...
        if not list_paths:
            raise FileNotFoundError(
                "No df_output*.parquet / df_output*.csv under %s" % path
            )
...
def _align_pair(
    ...
    if variable not in df_a.columns or variable not in df_b.columns:
        raise KeyError("Variable %r missing from one or both inputs." % variable)
...
def _build_text_message(data: Dict[str, Any]) -> str:
    ...
    for var, metrics in (data.get("per_variable") or {}).items():
        lines.append(
            "  %-6s rmse=%.4g bias=%.4g r=%.4g n=%s"
            % (var, metrics["rmse"], metrics["bias"], metrics["r"], metrics["n"])
        )
    return "\n".join(lines)
...
def compare_runs_cmd(
    ...
    if list_unknown:
        message = "Unknown metric(s): %s. Valid: %s" % (
            ", ".join(list_unknown),
    ...
    try:
        from ..metrics import bias as metric_bias
        from ..metrics import pearson_r, rmse as metric_rmse
        ...
        message = "Failed to load inputs: %s" % exc
        if json_mode:
            Envelope.error(
                errors=[message],
                command=command,
    ...
    for var in list_variables:
        try:
            ...
            list_warnings.append("variable %r skipped: %s" % (var, exc))
            continue
    ...
    if json_mode:
        ...
        for warn in list_warnings:
            click.echo("warning: %s" % warn, err=True)
