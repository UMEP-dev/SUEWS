...
class TestCompleteWorkflows:
    ...
    def comprehensive_forcing_data(self):
        ...
        solar_cycle = np.maximum(
            0, 
            solar_base * np.sin(np.pi * (hours % 24) / 24) * 
        ...
        rh_base = np.clip(75 - 0.8 * temp_diurnal + np.random.normal(0, 10, n_hours), 30, 95)
        ...
        rain_values = np.where(rain_prob < 0.05, np.random.exponential(2.0, n_hours), 0.0)
        ...
        data = {
            ...
            'ldown': 350 + 20 * np.sin(2 * np.pi * (hours % 24) / 24) + np.random.normal(0, 20, n_hours),
    ...
    async def test_complete_setup_to_analysis_workflow(
        ...
        comprehensive_forcing_data.to_csv(forcing_file, sep=' ', index=False, float_format='%.2f')
        ...
        if e2e_config.enable_simulation_tool:
            ...
            with patch.object(handlers, '_call_simulation_tool') as mock_simulate:
                ...
                assert "e2e_workflow_test_001" in simulation_result["content"][0]["text"]
    ...
    @pytest.mark.e2e
    @pytest.mark.asyncio
    async def test_multi_step_error_recovery_workflow(
        ...
        corrected_validation_args = {"config_file": str(corrected_config_file), "strict": True}
        ...
        with patch.object(handlers, '_call_validation_tool') as mock_validate:
            ...
            result = await handlers.handle_call_tool("validate_suews_config", corrected_validation_args)
        ...
        with patch.object(handlers, '_call_simulation_tool') as mock_simulate:
            ...
            result = await handlers.handle_call_tool("run_suews_simulation", corrected_simulation_args)
    ...
    async def test_concurrent_multi_step_workflows(self, e2e_config, tmp_path):
        ...
        assert "Active simulations: 0/3" in health_text or "Completed simulations: 3" in health_text
    ...
    async def test_data_preprocessing_workflow(
        ...
        assert any('invalid' in itype.lower() or 'range' in itype.lower() for itype in issue_types)
        ...
        for col in critical_cols:
            if col in processed_data.columns:
                assert not processed_data[col].isna().any(), f"Column {col} still has missing values"
