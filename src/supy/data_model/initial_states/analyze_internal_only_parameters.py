...
class InternalOnlyAnalyzer:
    ...
    def __init__(self):
        ...
        self.sample_config_path = (
            self.repo_root / "src" / "supy" / "sample_run" / "sample_config.yml"
        )
    ...
    def _extract_internal_params_from_file(
        self, file_path: Path
    ) -> List[Dict[str, Any]]:
            ...
            if field_match:
                ...
                if "internal_only" in field_def and "True" in field_def:
                    ...
                    desc_match = re.search(
                        r'description=["\'](.*?)["\']', field_def, re.DOTALL
                    )
                    description = (
    ...
    def _extract_complete_field_definition(
    ...
    def check_parameter_in_sample_config(
        self, param_name: str, yaml_keys: Set[str]
    ) -> Tuple[bool, str]:
    ...
    def _generate_markdown_report(self) -> None:
        ...
        in_sample = sum(
            1 for p in self.internal_params if p["in_sample_config"] == "Yes"
        )
...
""")
            ...
            for file_name, params in sorted(by_file.items()):
                ...
                for param in sorted(params, key=lambda x: x["param_name"]):
                    f.write(
                        f"| `{param['param_name']}` | {param['in_sample_config']} |\n"
                    )
...
""")
            ...
            present_params = [
                p for p in self.internal_params if p["in_sample_config"] == "Yes"
            ]
            if present_params:
            ...
            absent_params = [
                p for p in self.internal_params if p["in_sample_config"] == "No"
            ]
            if absent_params:
                f.write("| Parameter | File |\n")
                f.write("|-----------|------|\n")
