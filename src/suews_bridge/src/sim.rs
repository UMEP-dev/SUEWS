...
pub fn run_simulation_scm(
    ...
    unsafe {
        ffi::suews_scm_multitsteps_c(
            ...
            i32_len(input.forcing_block.len(), "forcing buffer length")?,
            len_sim_i32,
            forcing_cols_i32,
            input.nlayer,
            input.ndepth,
            ...
            bg_nz_i32,
            bg_t_buf.as_ptr(),
            i32_len(bg_t_buf.len(), "background time buffer length")?,
            bg_z_buf.as_ptr(),
            i32_len(bg_z_buf.len(), "background level buffer length")?,
            bg_theta_buf.as_ptr(),
            bg_q_buf.as_ptr(),
            i32_len(bg_theta_buf.len(), "background value buffer length")?,
