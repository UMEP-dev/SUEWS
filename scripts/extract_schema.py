#!/usr/bin/env python3
"""
Extract Schema without Building SUPY

This script extracts only the necessary data model files to a temporary location
and generates the schema from there, completely avoiding the main supy package.
"""
...
def copy_data_model(src_dir: Path, temp_dir: Path) -> None:
    """Copy only the data_model directory to temp location."""
...
def generate_schema_standalone() -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Copy data_model
        copy_data_model(src_dir, temp_path)
        ...
        try:
            ...
            print(
                f"✓ Generated schema with {len(schema.get('properties', {}))} properties"
            )
            ...
            index_content = registry.generate_index_html(
                base_url="https://umep-dev.github.io/SUEWS", is_preview=False
            )
