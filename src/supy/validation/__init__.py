...
def validate(paths: Iterable[str], schema_version: str | None = None) -> List[Dict[str, Any]]:
...
def phases(yaml_file: str, phase: str = "ABC", mode: str = "public") -> Tuple[bool, Dict[str, str]]:
    ...
    (uptodate, reportA, sci_yaml, reportB, pyd_yaml, reportC, _dir) = _proc_setup_output_paths(user_yaml, phase)

    outputs: Dict[str, str] = {}

    if phase == "A":
        ok = _proc_run_phase_a(user_yaml, standard_yaml, uptodate, reportA, mode=mode, phase="A", silent=True)
        outputs.update({"report": reportA, "yaml": uptodate})
        return ok, outputs

    if phase == "B":
        ok = _proc_run_phase_b(user_yaml, user_yaml, standard_yaml, sci_yaml, reportB, None, phase_a_performed=False, mode=mode, phase="B", silent=True)
        outputs.update({"report": reportB, "yaml": sci_yaml})
        return ok, outputs

    if phase == "C":
        ok = _proc_run_phase_c(user_yaml, pyd_yaml, reportC, mode=mode, phases_run=["C"], silent=True)
        outputs.update({"report": reportC, "yaml": pyd_yaml})
        return ok, outputs

    if phase == "AB":
        a_ok = _proc_run_phase_a(user_yaml, standard_yaml, uptodate, reportA, mode=mode, phase="AB", silent=True)
        b_ok = False
        if a_ok:
            b_ok = _proc_run_phase_b(user_yaml, uptodate, standard_yaml, sci_yaml, reportB, reportA, phase_a_performed=True, mode=mode, phase="AB", silent=True)
        outputs.update({"report": reportB, "yaml": sci_yaml})
        return a_ok and b_ok, outputs

    if phase == "AC":
        a_ok = _proc_run_phase_a(user_yaml, standard_yaml, uptodate, reportA, mode=mode, phase="AC", silent=True)
        c_ok = False
        if a_ok:
            c_ok = _proc_run_phase_c(uptodate, pyd_yaml, reportC, mode=mode, phase_a_report_file=reportA, phases_run=["A", "C"], silent=True)
        outputs.update({"report": reportC, "yaml": pyd_yaml})
        return a_ok and c_ok, outputs

    if phase == "BC":
        b_ok = _proc_run_phase_b(user_yaml, user_yaml, standard_yaml, sci_yaml, reportB, None, phase_a_performed=False, mode=mode, phase="BC", silent=True)
        c_ok = False
        if b_ok:
            c_ok = _proc_run_phase_c(sci_yaml, pyd_yaml, reportC, mode=mode, phases_run=["B", "C"], silent=True)
        outputs.update({"report": reportC, "yaml": pyd_yaml})
        return b_ok and c_ok, outputs

    # Default ABC
    a_ok = _proc_run_phase_a(user_yaml, standard_yaml, uptodate, reportA, mode=mode, phase="ABC", silent=True)
    b_ok = False
    c_ok = False
    if a_ok:
        b_ok = _proc_run_phase_b(user_yaml, uptodate, standard_yaml, sci_yaml, reportB, reportA, phase_a_performed=True, mode=mode, phase="ABC", silent=True)
    if a_ok and b_ok:
        c_ok = _proc_run_phase_c(sci_yaml, pyd_yaml, reportC, mode=mode, phase_a_report_file=None, phases_run=["A", "B", "C"], silent=True)
    outputs.update({"report": reportC, "yaml": pyd_yaml})
