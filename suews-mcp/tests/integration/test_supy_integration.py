...
class TestSuPyModelIntegration:
    ...
    def realistic_forcing_data(self):
        ...
        solar_cycle = np.maximum(0, 1000 * np.sin(np.pi * (hours % 24) / 24) * 
    ...
    async def test_list_tools_integration(self, integration_config):
        ...
        if integration_config.enable_validation_tool:
            assert any('validate' in name.lower() or 'configure' in name.lower() 
    ...
    async def test_simulation_workflow_with_mock_supy(
        self, mock_run_supy, mock_load_sample, integration_config, 
        realistic_forcing_data, suews_config_dict, tmp_path
    ):
        ...
        realistic_forcing_data.to_csv(forcing_file, sep=' ', index=False, float_format='%.2f')
    ...
    async def test_error_handling_in_integration(self, integration_config, tmp_path):
        ...
        with patch.object(handlers, '_call_simulation_tool') as mock_call:
            ...
            result = await handlers.handle_call_tool('run_suews_simulation', invalid_args)
    ...
    @pytest.mark.skipif(not SUPY_MCP_TOOLS_AVAILABLE, 
                       reason="SuPy MCP tools not available")
...
class TestSuPyDataIntegration:
    ...
    def test_data_structure_compatibility(self, realistic_forcing_data, sample_supy_config):
    ...
    @patch('supy.load_SampleData')
    def test_supy_data_loading_mock(self, mock_load, realistic_forcing_data, sample_supy_config):
