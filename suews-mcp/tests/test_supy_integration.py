...
class TestSuPyMCPIntegration:
    ...
    @pytest.mark.asyncio
    async def test_configure_tool_load_benchmark(
        self, configure_tool, benchmark_config_path
    ):
        ...
        if result["success"]:
            ...
            print(
                f"Config loading failed (expected in test environment): {result['errors']}"
            )
    ...
    async def test_configure_tool_default_config(self, configure_tool, temp_output_dir):
        ...
        if result["success"]:
            ...
            print(
                f"Default config creation failed (may be expected): {result.get('errors', [])}"
            )
    ...
    async def test_configure_tool_config_updates(self, configure_tool):
        ...
        if result["success"]:
            ...
            print(
                f"Config updates failed (may be expected): {result.get('errors', [])}"
            )
    ...
    @pytest.mark.asyncio
    async def test_run_tool_with_forcing_file(
        self, run_tool, benchmark_forcing_path, temp_output_dir
    ):
    ...
    async def test_tool_parameter_validation(
        self, configure_tool, run_tool, analyze_tool
    ):
    ...
    async def test_end_to_end_workflow(
        self, configure_tool, run_tool, analyze_tool, temp_output_dir
    ):
        ...
        if not config_result["success"]:
            pytest.skip(
                f"Configuration creation failed: {config_result.get('errors', [])}"
            )
...
class TestBenchmarkDataAvailability:
    ...
    def test_benchmark_config_exists(self):
        ...
        if config_path.exists():
            ...
            print(
                f"Benchmark config not found (expected in test environment): {config_path}"
            )
    ...
    def test_benchmark_forcing_exists(self):
        ...
        if forcing_path.exists():
            ...
            print(
                f"Benchmark forcing data not found (expected in test environment): {forcing_path}"
            )
