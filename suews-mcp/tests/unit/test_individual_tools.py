...
class TestHealthCheckTool:
    ...
    async def test_health_check_simulation_tracking(self, handlers):
        ...
        handlers._active_simulations = {
            ...
            "sim3": {"status": "failed", "config_file": "test3.yml", "error": "test error"},
...
class TestListResourcesTool:
    ...
    async def test_list_resources_filtered_types(self, handlers, tmp_path):
        ...
        with patch.object(Path, 'parent', new_callable=lambda: tmp_path):
            ...
            result = await handlers._list_resources_tool({"resource_type": "config_template"})
            text = result["content"][0]["text"]
...
class TestValidationTools:
    ...
    async def test_validate_suews_config_basic(self, handlers):
        ...
        result = await handlers._validate_config_tool({
            "config_file": "/path/to/test_config.yml",
            "strict": False
        })
    ...
    @pytest.mark.asyncio
    @patch("suews_mcp.handlers.ConfigValidator")
    async def test_validate_config_enhanced_with_errors(self, mock_validator_class, handlers):
...
class TestSimulationTool:
    ...
    async def test_run_simulation_concurrency_limiting(self, test_config):
        ...
        with patch('asyncio.sleep', mock_sleep):
            ...
            import asyncio
            task1 = asyncio.create_task(handlers._run_simulation_tool({
                "config_file": "/path/to/config1.yml",
                "simulation_id": "sim1"
            }))
            task2 = asyncio.create_task(handlers._run_simulation_tool({
                "config_file": "/path/to/config2.yml", 
                "simulation_id": "sim2"
            }))
...
class TestSuPyMCPToolsIntegration:
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    @pytest.mark.skipif(not SUPY_MCP_TOOLS_AVAILABLE, reason="SuPy MCP tools not available")
    ...
    async def test_supy_tool_execution_mock(self, handlers):
        ...
        mock_configure_tool.execute = AsyncMock(return_value={
            "success": True,
