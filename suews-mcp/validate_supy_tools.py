...
async def validate_supy_tools():
    ...
    try:
        ...
        print(
            f"✓ Configure tool: {config_def['name']} - {len(config_def['inputSchema']['properties'])} parameters"
        )
        print(
            f"✓ Run tool: {run_def['name']} - {len(run_def['inputSchema']['properties'])} parameters"
        )
        print(
            f"✓ Analyze tool: {analyze_def['name']} - {len(analyze_def['inputSchema']['properties'])} parameters"
        )
        ...
        print(
            f"✓ Configure tool execution: {'Success' if config_result.get('success', False) else 'Handled gracefully'}"
        )
        ...
        if run_result.get("success", False):
            ...
            print(
                f"⚠ Run tool execution: {run_result.get('errors', ['Unknown issue'])[:1]}"
            )
        ...
        try:
            analyze_result = await analyze_tool.execute(
                {"results_path": test_file, "analysis_type": "summary"}
            )
            if analyze_result.get("success", False):
                print("✓ Analyze tool execution: Success")
            else:
                print(
                    f"⚠ Analyze tool execution: {analyze_result.get('errors', ['Unknown issue'])[:1]}"
                )
        finally:
            os.unlink(test_file)
...
async def main():
    ...
    if supy_available:
        ...
        print(
            "⚠ This is expected in development - full functionality requires SuPy installation"
        )
