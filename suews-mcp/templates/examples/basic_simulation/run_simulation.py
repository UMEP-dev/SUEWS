...
async def run_basic_simulation():
    ...
    server_params = StdioServerParameters(
        command="python", args=["-m", "suews_mcp.server"]
    )
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                if sim_data.get("success", False):
                    ...
                    print(
                        f"   Duration: {sim_data.get('duration_seconds', 'N/A')} seconds"
                    )
                ...
                if analysis_data.get("success", False):
                    ...
                    if stats:
                        ...
                        for var, var_stats in stats.items():
                            ...
                            print(
                                f"   {var}: Mean = {mean_val:.2f} W/m²"
                                if var.startswith("Q")
                                else f"   {var}: Mean = {mean_val:.2f}"
                            )
                ...
                if not guidance_result.isError:
                    ...
                    print(
                        "ℹ️  Basic interpretation: Check seasonal patterns in QN, QH, QE"
                    )
