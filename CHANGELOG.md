...
- [bugfix] The xdist metrics test sleeps long enough for the CPU clock's tick to be small against it: `os.times()` quantises to 15.6 ms on Windows, so a 10 ms sleep could report more CPU than wall time and fail the merge queue's Windows API lane (#1796)
