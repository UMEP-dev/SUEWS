...
- [maintenance] `test_xdist_records_assignments_busy_duration_and_finish_skew` allows one tick of the coarsest CPU clock (1/64 s, the Windows clock-interrupt interval) when checking that a sleeping test's per-phase CPU seconds do not exceed its wall seconds: `os.times()` accrues CPU in whole scheduler ticks while `report.duration` is `perf_counter`, so a 10 ms sleeping phase could be charged a full 15.6 ms tick and fail the strict comparison, as it did on the Windows cp314 api lane in run 34455736441 (#1797)
