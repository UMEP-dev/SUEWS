...
def annotate_missing_parameters_refactored(
    ...
    reporter = build_phase_a_reporter(
        missing_params, renamed_replacements, extra_params, mode
    )
    ...
    if report_file:
        ...
        text_report = generate_phase_a_text_report(
...
def generate_phase_a_text_report(
        ...
        if critical_count > 0:
            ...
            for error in json_data.get("errors", []):
                if error.get("type") == "missing_required_parameter":
                    param_name = error.get("details", {}).get(
    ...
    has_no_action_items = (
        optional_count > 0
        or renamed_count > 0
        or (mode != "public" and extra_count > 0)
    )
    ...
    if has_no_action_items:
        ...
        if optional_count > 0:
            ...
            for info in json_data.get("info", []):
                if info.get("type") == "missing_optional_parameter":
                    param_name = info.get("details", {}).get(
                        "parameter_name", "unknown"
                    )
                    lines.append(
        ...
        if mode != "public" and extra_count > 0:
            ...
            if allowed_extras:
                lines.append(
                    f"- Found ({len(allowed_extras)}) parameter(s) not in standard:"
                )
