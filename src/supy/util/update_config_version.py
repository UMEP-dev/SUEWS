...
def update_yaml_config(
    ...
    try:
        ...
        if auto_increment and 'config_version' in config:
            config_version = increment_config_version(config.get('config_version', 'v1.0'))
        ...
        if update_name and 'name' in config:
            base_name = config['name'].split('_v')[0] if '_v' in config['name'] else config['name']
...
def main():
    ...
    parser.add_argument(
        "--config-version",
        help="Configuration schema version (e.g., v1.0, v1.1)"
    )
    parser.add_argument(
        "--auto-increment",
        action="store_true",
