...
def avg_t_air(
    cru_csv_path: Union[str, Path], lat: float, lon: float, tolerance: float = 0.5
) -> Dict[str, float]:
    """
    ...
    if len(nearby_data) > 12:  # More than one grid cell
        distances = np.sqrt(
            (nearby_data["Latitude"] - lat) ** 2 + (nearby_data["Longitude"] - lon) ** 2
        )
        ...
        grid_mask = (df["Latitude"] == closest_lat) & (df["Longitude"] == closest_lon)
...
def avg_t_air_from_yaml(
    yaml_path: Union[str, Path], cru_csv_path: Union[str, Path], tolerance: float = 0.5
) -> Dict[str, float]:
    """
...
def precheck_avg_t_air(
    yaml_path: Union[str, Path], tolerance: float = 0.5
) -> Dict[str, float]:
    """
