    ...
    def generate_dataset(n_hours: int = 8760, n_variables: int = 15) -> pd.DataFrame:
        ...
        for i, var in enumerate(extra_vars[: max(0, n_variables - len(data))]):
            if var in ["ldown"]:
                ...
                seasonal_lai = 2.0 + 2.0 * np.sin(
                    2 * np.pi * hours / (365 * 24) + np.pi / 2
                )
                data[var] = np.maximum(
                    0.5, seasonal_lai + np.random.normal(0, 0.3, n_hours)
                )
            else:
        ...
        output_data = pd.DataFrame(
            {
                ...
                "site_id": np.tile(range(1, n_sites + 1), n_hours // n_sites + 1)[
                    :n_hours
                ],
...
def workflow_test_data(tmp_path, large_forcing_dataset, comprehensive_suews_config):
    ...
    def create_workflow_data(
        scenario: str = "standard", duration_days: int = 7, n_sites: int = 1
    ) -> Dict[str, Any]:
