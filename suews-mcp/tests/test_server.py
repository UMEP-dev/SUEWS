...
class TestSUEWSMCPHandlers:
    ...
    def test_handlers_initialization(self, test_config):
        ...
        assert handlers._simulation_semaphore._value == test_config.max_concurrent_simulations
    ...
    async def test_handle_list_tools_full_config(self, handlers):
        ...
        expected_tools = {"run_suews_simulation", "validate_suews_config", "analyze_suews_output", "health_check"}
    ...
    @pytest.mark.unit
    @pytest.mark.handlers
    async def test_handle_call_tool_missing_arguments(self, handlers, invalid_tool_arguments):
    ...
    async def test_simulation_concurrency_limits(self, test_config):
        ...
        task1 = asyncio.create_task(
            handlers.handle_call_tool("run_suews_simulation", {"config_file": "test1.yml"})
        )
        ...
        task2 = asyncio.create_task(
            handlers.handle_call_tool("run_suews_simulation", {"config_file": "test2.yml"})
        )
...
class TestConfiguration:
    ...
    def test_config_validation(self):
        ...
        config = MCPServerConfig(log_level="INFO", suews_timeout=60, max_concurrent_simulations=2)
        assert config.log_level == "INFO"
        ...
        with pytest.raises(ValueError, match="Max concurrent simulations must be positive"):
...
class TestErrorHandling:
    ...
    async def test_server_error_handling_in_run(self, test_config, mock_stdio_server):
        ...
        with patch('suews_mcp.server.stdio_server', return_value=mock_stdio_server):
            with patch.object(server.server, 'run', side_effect=RuntimeError("Server error")):
    ...
    async def test_tool_error_propagation(self, handlers):
        ...
        handlers._run_simulation_tool = AsyncMock(side_effect=ValueError("Invalid config"))
        
        result = await handlers.handle_call_tool("run_suews_simulation", {"config_file": "test.yml"})
