module module_type_hydro
   ...
   TYPE, PUBLIC :: HYDRO_STATE
      ...
      REAL(KIND(1D0)), DIMENSION(nsurf) :: soil_store_surf = 0.0D0 ! Initial water stored in soil beneath `Bldgs` surface
      ...
      REAL(KIND(1D0)), DIMENSION(:), ALLOCATABLE :: soil_store_roof ! Soil moisture of roof [mm]
      REAL(KIND(1D0)), DIMENSION(:), ALLOCATABLE :: state_roof ! wetness status of roof [mm]
      REAL(KIND(1D0)), DIMENSION(:), ALLOCATABLE :: soil_store_wall ! Soil moisture of wall [mm]
      ...
      REAL(KIND(1D0)), DIMENSION(NSURF) :: runoff_soil = 0.0D0 !Soil runoff from each soil sub-surface [mm]
      ...
      REAL(KIND(1D0)) :: runoff_ag_veg = 0.0D0 !Above ground runoff from vegetated surfaces for all surface area [mm]
      REAL(KIND(1D0)) :: runoff_ag_impervious = 0.0D0 !Above ground runoff from impervious surface for all surface area [mm]
      REAL(KIND(1D0)) :: runoff_per_tstep = 0.0D0 !runoff water at each time step [mm]
      REAL(KIND(1D0)) :: runoff_pipes = 0.0 !runoff to pipes [mm]
      REAL(KIND(1D0)) :: runoff_soil_per_tstep = 0.0D0 !Runoff to deep soil per timestep [mm] (for whole surface, excluding water body)
      REAL(KIND(1D0)) :: runoff_waterbody = 0.0D0 !Above ground runoff from water body for all surface area [mm]
      REAL(KIND(1D0)) :: smd = 0.0D0 !soil moisture deficit [mm]
      REAL(KIND(1D0)) :: soil_state = 0.0D0 !Area-averaged soil moisture  for whole surface [mm]
      ...
      REAL(KIND(1D0)) :: nw_state_per_tstep = 0.0D0 ! state_id at each tinestep(excluding water body) [mm]

      REAL(KIND(1D0)) :: soil_moist_cap !Maximum capacity of soil store [mm]
      ...
      REAL(KIND(1D0)) :: additional_water = 0.0D0 !!Additional water coming from other grids [mm] (these are expressed as depths over the whole surface)
      REAL(KIND(1D0)) :: add_impervious = 0.0D0
      REAL(KIND(1D0)) :: add_pipes = 0.0D0
      REAL(KIND(1D0)) :: add_veg = 0.0D0
      REAL(KIND(1D0)) :: add_water_body = 0.0D0
...
CONTAINS
   ...
   SUBROUTINE allocHydroState_c(self, nlayer)
      ...
      ALLOCATE (self%soil_store_roof(nlayer))
      ALLOCATE (self%state_roof(nlayer))
      ALLOCATE (self%soil_store_wall(nlayer))
   ...
   SUBROUTINE deallocHydroState_c(self)
      ...
      IF (ALLOCATED(self%soil_store_roof)) DEALLOCATE (self%soil_store_roof)
      IF (ALLOCATED(self%state_roof)) DEALLOCATE (self%state_roof)
      IF (ALLOCATED(self%soil_store_wall)) DEALLOCATE (self%soil_store_wall)
