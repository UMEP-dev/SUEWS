...
#. Several sub-models are available to estimate the storage heat flux:
   ...
   -  **DyOHM** (Dynamic Objective Hysteresis Model) (Liu et al., in prep.). Calculates OHM coefficients dynamically from meteorological conditions and the outermost material layer. Method 6 applies DyOHM to all SUEWS land-cover surfaces: buildings use material layer 0 of ``land_cover.bldgs``, and each non-building land cover uses its own material layer 0. Method 7 uses STEBBS storage heat for buildings and DyOHM for non-building surfaces. Method 8 uses material layer 0 of ``land_cover.bldgs`` for building DyOHM and ordinary OHM for non-building surfaces. Roof and wall material properties are not used by DyOHM.
...
Sensible and latent heat fluxes from both LUMPS and SUEWS are provided in the :ref:`output files <output_files>`.
