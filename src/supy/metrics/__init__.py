...
def _coerce_pair(
    ...
    if len(df_pair) < _MIN_VALID_PAIRS:
        raise ValueError(
            "Need at least %d non-NaN aligned pairs for a metric; got %d."
            % (_MIN_VALID_PAIRS, len(df_pair))
        )
...
def pearson_r(
    ...
    """
    arr_true, arr_pred = _coerce_pair(y_true, y_pred)
    if np.std(arr_true) == 0.0 or np.std(arr_pred) == 0.0:
        return float("nan")
