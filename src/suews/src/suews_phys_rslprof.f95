...
!          Lc, beta, zd_RSL, z0_RSL, elm, Scc, fx)
      ASSOCIATE ( &
         ...
         ASSOCIATE ( &
            ...
            ! minimum canyon height is 2.0 m to avoid insane values and ensure proper interpolation at diagnostic heights
            zH_RSL = MAX(Zh, 2.)
   ...
   FUNCTION interp_z(z_x, z, v) RESULT(v_x)
      ...
      ELSE IF (idx_low == 0) THEN
         ! z_x is below all z values (extrapolate using first two points)
         IF (idx_high > 1) THEN
            ! Use first two distinct z values for extrapolation
            dz = z(2) - z(1)
            IF (ABS(dz) < 1.D-10) THEN
               ! First two points have same z, use constant value
               v_x = v(1)
            ELSE
               slope = (v(2) - v(1))/dz
               v_x = v(1) + (z_x - z(1))*slope
            END IF
         ELSE
            ! All z values are above z_x, use first value
            v_x = v(1)
         END IF
      ELSE IF (idx_high == 0) THEN
         ! z_x is above all z values (extrapolate using last two points)
         dz = z(nz) - z(nz-1)
         IF (ABS(dz) < 1.D-10) THEN
            ! Last two points have same z, use constant value
            v_x = v(nz)
         ELSE
            slope = (v(nz) - v(nz-1))/dz
            v_x = v(nz-1) + (z_x - z(nz-1))*slope
         END IF
      ELSE
