...
def calculate_pet_simple(t_air: pd.Series, rh: pd.Series, ws: pd.Series, tmrt: Optional[pd.Series] = None) -> pd.Series:
...
async def interpret_thermal_comfort(output_path: str, period: str = "full") -> str:
    ...
    try:
        ...
        if path.suffix == '.csv':
            ...
            df['datetime'] += pd.to_timedelta(df['Hour'], unit='h') + pd.to_timedelta(df['Min'], unit='m')
        ...
        metrics = {
            ...
            'apparent_temp': calculate_apparent_temp(df_period['T2'], df_period['RH2'], ws[df_period.index]),
        ...
        if tmrt is not None:
            metrics['pet'] = calculate_pet_simple(
                df_period['T2'], df_period['RH2'], 
                ws[df_period.index], tmrt[df_period.index]
            )
        ...
        if 'pet' in metrics:
            ...
            output += f"- Hours in comfort zone (PET 18-23°C): {comfortable:,} ({comfort_pct:.1f}%)\n"
