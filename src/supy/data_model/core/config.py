...
class SUEWSConfig(BaseModel):
    ...
    def _needs_storage_validation(self) -> bool:
        ...
        -------
        bool
            True if `storage_heat_method` is set to 6, 7, or 16 and was explicitly configured by the user,
            False otherwise.

        Notes
        -----
        - Validation is only triggered if `storage_heat_method` is 6, 7, or 16 AND the value was explicitly set
          (not just the default value).
        - Uses Pydantic's `model_fields_set` to distinguish user-provided values from defaults.
        """
        if not hasattr(self.model, "physics") or not hasattr(
        ...
        # Only validate if method == 6, 7, or 16 AND it was explicitly set
        if shm in {6, 7, 16}:
    ...
    def _validate_storage(self, site: Site, site_index: int) -> List[str]:
        ...
        method (storage_heat_method 6, 7, or 16) are present and valid for the given site.
        ...
        if not walls or len(walls) == 0:
            issues.append(
                f"{site_name}: storage_heat_method 6, 7, or 16 (DyOHM) selected → missing vertical_layers.walls"
            )
            return issues
        ...
        for arr in ("dz", "k", "rho_cp"):
            ...
            if (
                ...
                issues.append(
                    f"{site_name}: storage_heat_method 6, 7, or 16 (DyOHM) selected → "
        ...
        if lam in (None, ""):
            issues.append(
                f"{site_name}: storage_heat_method 6, 7, or 16 (DyOHM) selected → properties.lambda_c must be set and non-null"
            )
