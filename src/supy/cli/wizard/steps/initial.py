...
class InitialConditionsStep(WizardStep):
    ...
    def collect_input(self) -> Dict[str, Any]:
        ...
        use_defaults = Confirm.ask(
            "\nWould you like to use default initial conditions?", default=True
        )
        ...
        if use_defaults:
            ...
            initial_conditions["air_temperature"] = FloatPrompt.ask(
                "Initial air temperature (°C)", default=self.defaults["air_temperature"]
            )
            ...
            console.print(
                "Enter volumetric soil moisture content (0-1) for each surface:"
            )
            ...
            surface_fractions = self.session.configuration.get("surface", {}).get(
                "fractions", {}
            )
            ...
            for surface in surfaces:
                ...
                if (
                    surface_key in surface_fractions
                    and surface_fractions[surface_key] > 0
                ):
                    ...
                    initial_conditions["soil_moisture"][surface] = self.defaults[
                        "soil_moisture"
                    ][surface]
            ...
            for surface in surfaces:
                ...
                if (
                    ...
                    initial_conditions["surface_temperature"][surface] = (
                        FloatPrompt.ask(
                    ...
                    initial_conditions["surface_temperature"][surface] = self.defaults[
                        "surface_temperature"
                    ][surface]
            ...
            if has_snow:
                initial_conditions["snow_water_equivalent"] = FloatPrompt.ask(
                    "Snow water equivalent (mm)", default=10.0
                )

                initial_conditions["snow_albedo"] = FloatPrompt.ask(
                    "Snow albedo (0-1)", default=0.7
                )

                initial_conditions["snow_density"] = FloatPrompt.ask(
                    "Snow density (kg/m³)", default=200.0
                )
            else:
    ...
    def _show_initial_conditions_summary(self, conditions: Dict[str, Any]):
        ...
        avg_soil_moisture = sum(conditions["soil_moisture"].values()) / len(
            conditions["soil_moisture"]
        )
        table.add_row("Avg. Soil Moisture", f"{avg_soil_moisture:.2f}")

        # Surface temperature (average)
        avg_surface_temp = sum(conditions["surface_temperature"].values()) / len(
            conditions["surface_temperature"]
        )
        table.add_row("Avg. Surface Temp", f"{avg_surface_temp:.1f} °C")

        # Snow
        table.add_row(
            "Snow Water Equiv.", f"{conditions['snow_water_equivalent']:.1f} mm"
        )
    ...
    def validate(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for surface, moisture in data.get("soil_moisture", {}).items():
                if not 0 <= moisture <= 1:
                    console.print(
                        f"[red]Soil moisture for {surface} must be between 0 and 1[/red]"
                    )
                    return False
