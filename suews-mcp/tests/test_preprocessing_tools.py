...
class TestPreprocessingToolsIntegration:
    ...
    async def test_list_tools_includes_preprocessing(self, handlers):
        ...
        preprocess_tool = next(
            tool for tool in result.tools if tool.name == "preprocess_forcing"
        )
        assert "meteorological forcing data" in preprocess_tool.description.lower()

        validate_tool = next(
            tool for tool in result.tools if tool.name == "validate_config"
        )
        assert "configuration" in validate_tool.description.lower()

        convert_tool = next(
            tool for tool in result.tools if tool.name == "convert_data_format"
        )
        assert "format" in convert_tool.description.lower()

    @pytest.mark.asyncio
    async def test_preprocess_forcing_tool_success(
        self, handlers, sample_forcing_data, tmp_path
    ):
    ...
    async def test_preprocess_forcing_tool_invalid_file(self, handlers):
        ...
        arguments = {"input_file": "/nonexistent/file.txt"}
    ...
    async def test_validate_config_tool_missing_file(self, handlers):
        ...
        arguments = {"config_file": "/nonexistent/config.yml"}
    ...
    @pytest.mark.asyncio
    async def test_convert_data_format_tool_success(
        self, handlers, sample_csv_data, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_convert_data_format_tool_suews_format(
        self, handlers, sample_csv_data, tmp_path
    ):
    ...
    async def test_preprocessing_tools_workflow(self, handlers, tmp_path):
        ...
        convert_result = await handlers.handle_call_tool(
            "convert_data_format", convert_args
        )
        ...
        preprocess_result = await handlers.handle_call_tool(
            "preprocess_forcing", preprocess_args
        )
        ...
        config = {
            ...
            "model": {
                ...
                "physics": {"netradiationmethod": {"value": 1}},
            },
            "sites": [
                {
                    "name": "test",
        ...
        validate_result = await handlers.handle_call_tool(
            "validate_config", validate_args
        )
    ...
    async def test_tool_schema_validation(self, handlers):
        ...
        preprocessing_tools = [
            ...
            if tool.name
            in ["preprocess_forcing", "validate_config", "convert_data_format"]
        ]
