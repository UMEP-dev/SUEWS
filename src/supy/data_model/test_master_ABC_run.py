...
def test_setup_output_paths_phase_a(test_env):
    ...
    uptodate_file, report_file, science_yaml_file, science_report_file, dirname = (
        setup_output_paths(test_env["test_yaml_file"], "A")
    )
    ...
    assert report_file == os.path.join(
        test_env["test_dir"], f"reportA_{name_without_ext}.txt"
    )
    assert science_yaml_file == os.path.join(
        test_env["test_dir"], f"updatedB_{basename}"
    )
    assert science_report_file == os.path.join(
        test_env["test_dir"], f"reportB_{name_without_ext}.txt"
    )
...
def test_setup_output_paths_phase_b(test_env):
    ...
    uptodate_file, report_file, science_yaml_file, science_report_file, dirname = (
        setup_output_paths(test_env["test_yaml_file"], "B")
    )
...
def test_setup_output_paths_phase_ab(test_env):
    ...
    uptodate_file, report_file, science_yaml_file, science_report_file, dirname = (
        setup_output_paths(test_env["test_yaml_file"], "AB")
    )
    ...
    assert uptodate_file == os.path.join(
        test_env["test_dir"], f"updatedA_{basename}"
    )  # Intermediate
    assert report_file == os.path.join(
        test_env["test_dir"], f"reportA_{name_without_ext}.txt"
    )  # Intermediate
    assert science_yaml_file == os.path.join(
        test_env["test_dir"], f"updatedAB_{basename}"
    )  # Final
    assert science_report_file == os.path.join(
        test_env["test_dir"], f"reportAB_{name_without_ext}.txt"
    )  # Final
...
def test_run_phase_a_success(mock_annotate, test_env):
    ...
    result = run_phase_a(
        test_env["test_yaml_file"],
        test_env["standard_yaml_file"],
        uptodate_file,
        report_file,
    )
...
def test_command_line_parsing(test_env):
    ...
    with patch("sys.argv", test_args):
        with patch(
            "master_ABC_run.validate_input_file",
            return_value=test_env["test_yaml_file"],
        ):
...
@patch("os.remove")
def test_file_cleanup_on_successful_ab_workflow(
    mock_remove, mock_exists, mock_phase_a, mock_phase_b, test_env
):
...
def test_phase_b_with_existing_phase_a_output(test_env):
    ...
    with patch("sys.argv", test_args):
        with patch(
            ...
            with patch("os.path.exists", return_value=True):
                with patch(
                    "master_ABC_run.run_phase_b", return_value=True
                ) as mock_phase_b:
                    result = main()
        ...
        with patch(
            ...
            with patch("os.path.exists", side_effect=mock_exists):
                with patch(
                    ...
                    assert (
                        call_args[0][1] == test_env["test_yaml_file"]
                    )  # Should use user file directly
    ...
    with patch("sys.argv", test_args):
        with patch(
            ...
            with patch("os.path.exists", return_value=True):
                with patch("master_ABC_run.run_phase_a", return_value=True):
                    with patch(
                        "master_ABC_run.run_phase_b", return_value=False
                    ):  # Phase B fails
...
def test_file_naming_conventions():
    ...
    # Test Phase B naming
    uptodate_file, report_file, science_yaml_file, science_report_file, _ = (
        setup_output_paths(os.path.join(test_dir, basename), "B")
    )
    ...
    # Test AB naming
    uptodate_file, report_file, science_yaml_file, science_report_file, _ = (
        setup_output_paths(os.path.join(test_dir, basename), "AB")
    )
...
@pytest.mark.parametrize(
    "phase,expected_yaml,expected_report",
    [
...
def test_phase_naming_patterns(phase, expected_yaml, expected_report):
    ...
    _, _, science_yaml_file, science_report_file, _ = setup_output_paths(
        test_file, phase
    )
