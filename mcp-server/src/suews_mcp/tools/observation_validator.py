...
def calculate_metrics(obs: pd.Series, mod: pd.Series) -> Dict[str, float]:
    ...
    # Linear regression
    slope = ((obs_clean - obs_mean) * (mod_clean - mod_mean)).sum() / ((obs_clean - obs_mean) ** 2).sum()
...
def interpret_metrics(metrics: Dict[str, float], variable: str) -> Dict[str, str]:
    ...
    var_thresholds = thresholds.get(variable, {"good_rmse": np.inf, "acceptable_rmse": np.inf, "units": ""})
    ...
    if sys_ratio > 0.5:
        interpretations["error_type"] = "Primarily systematic errors - check model physics/parameters"
    else:
...
def analyze_temporal_patterns(obs_df: pd.DataFrame, mod_df: pd.DataFrame, variable: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if model_path.suffix == '.csv':
            ...
            mod_df['datetime'] = pd.to_datetime(
                mod_df['Year'].astype(str) + mod_df['DOY'].astype(str), 
                format='%Y%j'
            )
            mod_df['datetime'] += pd.to_timedelta(mod_df['Hour'], unit='h') + \
                                 pd.to_timedelta(mod_df.get('Min', 0), unit='m')
        ...
        if obs_path.suffix == '.csv':
            ...
            if 'datetime' in obs_df.columns:
                ...
                date_cols = [col for col in obs_df.columns if col.lower() in ['year', 'month', 'day', 'hour', 'doy']]
                if 'Year' in obs_df.columns and 'DOY' in obs_df.columns:
                    obs_df['datetime'] = pd.to_datetime(
                        obs_df['Year'].astype(str) + obs_df['DOY'].astype(str), 
                        format='%Y%j'
                    )
        ...
        for var in variables:
            ...
            output += f"- **Regression**: y = {metrics['slope']:.2f}x + {metrics['intercept']:.2f}\n\n"
            ...
            if "hourly" in patterns and patterns["hourly"]:
                ...
                worst_hours = sorted(patterns["hourly"].items(), 
                                   key=lambda x: x[1]["rmse"], reverse=True)[:3]
