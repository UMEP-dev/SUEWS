...
- [change][stable] Temporarily disabled the legacy ESTM storage-heat scheme in the Fortran kernel, pending a per-grid redesign (#1802, #1741)
  - `suews_phys_estm.f95` is taken out of the build together with its driver dispatch and the `_ESTM_Ts_data.txt` column constants; the source stays in git history and `dev-ref/ESTM_REACTIVATION.md` records what was taken out, what must be fixed first, and how to bring it back. Only the `ESTM` subroutine was ever reachable, and since #1785 the option is refused before the kernel runs; every other procedure in the file had no caller. Disabling it takes out the last module-level run state and implicitly saved first-call counters found in the #1741 audit, so the saved-locals lint allowlist is now empty. `SUEWS_cal_Qs` now refuses any storage-heat value with no scheme behind it through the error state (code 106), and the driver stops the timestep straight after storage heat when a fatal error is pending, so a caller that bypasses the data model gets an exception instead of `QS = -999` in the energy balance. As a consequence the fatal error pending at that checkpoint is the one reported (the thread-local store is overwrite-on-set, so later errors in the same timestep can no longer replace it): a run with the measurement height below the displacement height now reports ErrorHint 14 from the roughness calculation rather than the later stability error 32 that used to overwrite it. The `ESTM` output group stays in the output contract.
...
- [maintenance] The api cross-CPython lane runs on xdist worksteal workers per platform (Linux 4, Windows 4, macOS 2, counts measured in #1786) with a `serial_platforms` escape hatch fed by the `api_serial_platforms` dispatch input or the `SUEWS_API_SERIAL_PLATFORMS` repository variable, carried caller -> `build-wheels-reusable.yml` -> api lane now that the lane is chained behind each platform's own wheel build (#1792); a new contract test pins the per-platform api gate to the quoting the platform presets in `determine-matrix.sh` are written in, so a preset or key that loses its quotes cannot skip every api lane while the publish gate stays green (#1788)
...
- [maintenance] The two MCP concurrency tests (gh#1412 guards) assert the contrast between the same two tool calls run one after the other and run concurrently on one session (two `query_knowledge` calls complete within half a serial call of each other instead of one whole call apart; for `query_knowledge` with a primed `search_schema`, the probe's extra delay behind the subprocess is below half the `query_knowledge` time), instead of a fixed 1.5x multiple of a single-call baseline that read CPU contention as a regression on the 3-core macOS runner and on loaded laptops; the per-task deadline becomes a hang guard at four times the serial total (floored at 30 s, capped at the 120 s request read-timeout), and each test logs its S and C values (#1793)
- [maintenance] The per-test CPU-versus-wall check in the CI metrics xdist test now sleeps for several scheduler quanta and allows one 15.625 ms tick of CPU-clock slack, so the Windows api lane no longer fails on the quantised `GetProcessTimes` reading introduced with the per-test cost records (#1779)
- [maintenance] Recorded what the nightly scheduled run does as the project's scientific tier and why the PR and merge-queue tiers stay fast (`.claude/rules/ci/conventions.md`, "The Nightly as the Scientific Tier"): each schedule-only job, the artefact it produces, who reads it, which jobs may redden the night, and the rule that a new expensive check goes to the nightly first as a recording job. The `report_scheduled_run` alert now counts any needed job whose result is not `success` as a failure, `skipped` included -- an upstream failure that skipped every build would otherwise have been reported as a green night and closed the tracking issue -- and `scripts/suews/nightly_streak.py` reads the consecutive-green streak in one command, listing failing job names and keeping the `continue-on-error` recording jobs out of the count (#1795)
- [maintenance] The legacy-table round-trip tests resolve the vendored fixture set without copying it and forward-convert each legacy version once per pytest session (`legacy_tables` and `forward_converted` fixtures) instead of rebuilding both per parametrised case; the file was the only one in the API lane materially above the platform-wide 1.35x Windows-to-Linux ratio, at 1.66x (36 s beyond what the baseline predicts), and it drops from 71.9 s to 66.8 s on a Linux machine with assertions unchanged (#1791)
...
- [maintenance] The "CI Build Plan" PR comment now describes the matrix and test tier from the `determine_matrix` outputs (a Matrix row classified from `buildplat`, a `physics-full` description) instead of inferring them from the draft flag; the body builder has a `node:test` suite run by the workflow-security workflow (#1773)
- [maintenance] The `medium` and `slow` cost markers are defined by process CPU seconds on the Linux reference runner instead of wall-clock feel: the pytest metrics plugin records wall and CPU seconds per test (additive to schema v2), `scripts/lint/check_cost_markers.py` names every test whose marker disagrees with its measured cost, and the nightly wheel workflow runs it as a reporting job (`cost_markers`, also on `workflow_dispatch`); three sample-run physics checks moved out of the api-marked `test/core/test_supy.py` into `test/physics/test_supy_sample_run.py` so they run once per OS rather than per CPython (#1779)
...
- [feature][experimental] Revived the AnOHM analytical storage-heat scheme (`StorageHeatMethod=3`), disabled since 2023, kept as an internal / not-recommended option (supersedes the stalled #1018)
  - Replaced the future-data forcing dependency with a calendar-day double buffer on `OHM_STATE`, so coefficients are diagnosed from the most recently completed day
  ...
  - Persisted both AnOHM buffers, their readiness state and the live per-surface coefficients through the versioned C/Rust `OHM_STATE` codec; version-1 payloads remain readable with default inactive AnOHM fields
  - Replaced the process-global coefficient `SAVE` cache with simulation-owned `OHM_STATE`, preventing sequential same-day/grid runs from sharing coefficients
  - Added `test/physics/test_anohm_revival.py` (runs end-to-end, engages AnOHM after buffer fill, and verifies checkpoint/chunk parity plus independent-run cache isolation)
...
- [change] Removed `BaseTMethod` parameter from HDD/CDD calculations (#749)
