...
from ....data_model.validation.core.controller import (
...
class EnhancedWizardValidator:
    ...
    def validate_complete_config(
        self, config_dict: Dict[str, Any], show_progress: bool = True
        ...
        try:
            ...
            for error in conditional_result.errors:
                self.report.add_issue(
                    ValidationLevel.ERROR, "physics_options", "conditional", error
                )

            for warning in conditional_result.warnings:
                self.report.add_issue(
                    ValidationLevel.WARNING, "physics_options", "conditional", warning
                )

            if show_progress and conditional_result.skipped_methods:
                console.print(
                    f"   [dim]Skipped methods: {', '.join(conditional_result.skipped_methods)}[/dim]"
                )
        ...
        phase_b_valid, phase_b_config, phase_b_errors = self._run_phase_b_validation(
            corrected_config
        )
        if phase_b_config:
        ...
        for error in phase_c_errors:
            self.report.add_issue(ValidationLevel.ERROR, "phase_c", "pydantic", error)
        ...
        try:
            ...
            for error in validator.iter_errors(config_dict):
                ...
                if "required" in error.message.lower():
                    fix_suggestion = (
                        f"Add the missing field '{error.message.split("'")[1]}'"
                    )
    ...
    def _run_conditional_validation(
        ...
        result = ValidationResult(
            status="passed", validated_methods=set(), skipped_methods=set()
        )
        ...
        if self.controller.roughness_variable_enabled:
            ...
            if sites:
                for surface in (
                    sites[0].get("properties", {}).get("land_cover", {}).values()
                ):
                    if (
                        "bldgh" not in surface
                        and surface.get("sfr", {}).get("value", 0) > 0
                    ):
                        result.warnings.append(
    ...
    def _display_summary(self):
        ...
        error_count = sum(
            1 for i in self.report.issues if i.level == ValidationLevel.ERROR
        )
        warning_count = sum(
            1 for i in self.report.issues if i.level == ValidationLevel.WARNING
        )
        info_count = sum(
            1 for i in self.report.issues if i.level == ValidationLevel.INFO
        )
    ...
    def get_fix_suggestions(self) -> List[str]:
        ...
        if any(
            "surface fraction" in str(i.message).lower() for i in self.report.issues
        ):
            suggestions.append(
    ...
    def export_json_report(self, filepath: Optional[Path] = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            "conditional_validation": {
                "enabled_methods": list(self.controller.validated_methods)
                if self.controller
                else [],
                "skipped_methods": list(self.controller.skipped_methods)
                if self.controller
                else [],
            },
        }
