...
class DataQualityIssue:
    ...
    def __init__(self, issue_type: str, message: str, severity: str = "warning", 
                 location: Optional[str] = None, value: Any = None):
...
class ForcingDataPreprocessor:
    ...
    def preprocess_forcing_file(
        ...
        try:
            ...
            self.result.add_issue(
                DataQualityIssue("processing_error", f"Failed to process file: {str(e)}", "error")
            )
    ...
    def _load_forcing_data(self, file_path: Path) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if file_path.suffix.lower() == '.csv':
                ...
                self.result.add_issue(
                    DataQualityIssue("file_format", f"Unsupported file format: {file_path.suffix}", "error")
                )
                return None
        ...
        try:
            ...
            for idx, row in df_temp.iterrows():
                try:
                    ...
                    date = datetime(int(row['iy']), 1, 1) + timedelta(days=int(row['id']) - 1)
                    time = datetime.combine(date.date(), datetime.min.time().replace(
            ...
            if invalid_dates:
                self.result.add_issue(
                    DataQualityIssue(
                        ...
                        "error",
                        location=f"rows: {invalid_dates[:10]}" + ("..." if len(invalid_dates) > 10 else "")
                    )
                )
            ...
            if len(df_temp) > 1:
                ...
                for i in range(1, min(100, len(df_temp))):  # Check first 100 rows
                    try:
                        dt1 = datetime(int(df_temp.iloc[i-1]['iy']), 1, 1) + timedelta(days=int(df_temp.iloc[i-1]['id']) - 1)
                        dt1 = dt1.replace(hour=int(df_temp.iloc[i-1]['it']), minute=int(df_temp.iloc[i-1]['imin']))
                        dt2 = datetime(int(df_temp.iloc[i]['iy']), 1, 1) + timedelta(days=int(df_temp.iloc[i]['id']) - 1)
                        dt2 = dt2.replace(hour=int(df_temp.iloc[i]['it']), minute=int(df_temp.iloc[i]['imin']))
                        time_diffs.append((dt2 - dt1).total_seconds())
                    except:
                        continue
                
                if time_diffs:
                    most_common_diff = pd.Series(time_diffs).mode().iloc[0] if time_diffs else None
                    if most_common_diff:
                        self.result.metadata['detected_timestep_seconds'] = most_common_diff
                        self.result.add_log(f"Detected time step: {most_common_diff} seconds")
        
        except Exception as e:
            self.result.add_issue(
                DataQualityIssue("time_validation", f"Error validating time series: {str(e)}", "warning")
            )
    ...
    def _check_missing_data(self, df: pd.DataFrame):
        ...
        for col in df.columns:
            if col in self.SUEWS_COLUMNS:
                ...
                if missing_count > 0:
                    severity = "error" if missing_pct > 50 else "warning" if missing_pct > 10 else "info"
                    self.result.add_issue(
                        DataQualityIssue(
                            "missing_data",
        ...
        if len(available_cols) >= 3:  # Need at least 3 components
            ...
            if len(df_energy) > 0:
                ...
                if 'qh' in available_cols and 'qe' in available_cols:
                    ...
                    if len(qe_nonzero) > 0:
                        bowen_ratio = (df_energy['qh'][df_energy['qe'] != 0] / qe_nonzero).median()
        ...
        for var in ['Tair', 'RH', 'kdown']:
            if var in df.columns and 'it' in df.columns:
                ...
                if len(hourly_stats) >= 12:  # Need at least 12 hours
                    ...
                    if var == 'Tair' and daily_range < 2.0:  # Less than 2°C diurnal range
                        self.result.add_issue(
                            DataQualityIssue(
                    ...
                    elif var == 'kdown' and daily_range < 50:  # Very low solar radiation variation
                        self.result.add_issue(
                            DataQualityIssue(
                                "weak_diurnal_pattern",
    ...
    def _apply_unit_conversions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'pres' in df_processed.columns:
            pres_values = df_processed['pres'][~df_processed['pres'].isin(self.MISSING_DATA_FLAGS)]
    ...
    def _auto_fix_data_issues(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for var in ['kdown', 'kdiff', 'kdir']:
            if var in df_fixed.columns:
                mask = (df_fixed[var] < 0) & (~df_fixed[var].isin(self.MISSING_DATA_FLAGS))
                if mask.sum() > 0:
                    df_fixed.loc[mask, var] = 0
                    self.result.add_log(f"Set {mask.sum()} negative {var} values to zero")
        ...
        if 'RH' in df_fixed.columns:
            ...
            if len(rh_values) > 0 and rh_values.max() <= 1.5:
                ...
                self.result.add_log("Converted relative humidity from fraction to percentage")
    ...
    def _adjust_timestep(self, df: pd.DataFrame, target_timestep: int) -> pd.DataFrame:
        ...
        self.result.add_log(f"Timestep adjustment to {target_timestep}s requested (simplified)")
        return df
    ...
    def _final_validation(self, df: pd.DataFrame):
        ...
        if missing_cols:
            self.result.add_issue(
                DataQualityIssue("final_validation", f"Missing required columns after processing: {missing_cols}", "error")
            )
        
        self.result.add_log("Final validation completed")
    
    def _generate_metadata(self, df_original: pd.DataFrame, df_processed: pd.DataFrame, file_path: Path) -> Dict[str, Any]:
        ...
        if len(numeric_cols) > 0:
            ...
            for col in numeric_cols:
                if col in self.SUEWS_COLUMNS:
                    non_missing = df_processed[col][~df_processed[col].isin(self.MISSING_DATA_FLAGS)]
    ...
    def _save_processed_data(self, df: pd.DataFrame, output_path: Union[str, Path]):
        ...
        try:
            ...
            self.result.add_issue(
                DataQualityIssue("save_error", f"Failed to save processed data: {str(e)}", "error")
            )
...
class ConfigValidator:
    ...
    def validate_config(
        ...
        try:
            ...
            self.result.add_issue(
                DataQualityIssue("validation_error", f"Failed to validate config: {str(e)}", "error")
            )
        ...
        try:
            with open(config_path, 'r') as f:
                if config_path.suffix.lower() in ['.yml', '.yaml']:
                    ...
                    self.result.add_issue(
                        DataQualityIssue("unsupported_format", f"Unsupported config format: {config_path.suffix}", "error")
                    )
                    return None
            
            self.result.add_log(f"Loaded configuration with {len(config_data)} top-level sections")
            return config_data
            
        except Exception as e:
            self.result.add_issue(
                DataQualityIssue("config_load_error", f"Cannot load config file: {str(e)}", "error")
            )
            return None
    ...
    def _validate_config_structure(self, config_data: Dict[str, Any]):
        ...
        if 'sites' in config_data:
            if not isinstance(config_data['sites'], list) or len(config_data['sites']) == 0:
                self.result.add_issue(
                    DataQualityIssue(
        ...
        if 'sites' in config_data:
            for i, site in enumerate(config_data['sites']):
                if 'properties' in site:
                    ...
                    # Latitude validation
                    if 'lat' in props:
                        lat_val = props['lat'].get('value') if isinstance(props['lat'], dict) else props['lat']
                        if not isinstance(lat_val, (int, float)) or lat_val < -90 or lat_val > 90:
                            self.result.add_issue(
                                DataQualityIssue(
                                    "invalid_latitude",
                    ...
                    if 'lng' in props:
                        lng_val = props['lng'].get('value') if isinstance(props['lng'], dict) else props['lng']
                        if not isinstance(lng_val, (int, float)) or lng_val < -180 or lng_val > 180:
                            self.result.add_issue(
                                DataQualityIssue(
                                    "invalid_longitude",
...
class DataFormatConverter:
    ...
    def convert_format(
        ...
        **kwargs
        ...
        try:
            ...
            self._save_data_by_format(df_converted, output_path, output_format, **kwargs)
            ...
            self.result.add_issue(
                DataQualityIssue("conversion_error", f"Failed to convert format: {str(e)}", "error")
            )
        
        return self.result
    
    def _load_data_by_format(self, file_path: Path, format_type: str, **kwargs) -> Optional[pd.DataFrame]:
        ...
        try:
            if format_type.lower() == 'csv':
                ...
                try:
                    ...
                    self.result.add_issue(
                        DataQualityIssue("missing_dependency", "xarray required for NetCDF support", "error")
                    )
                    return None
            else:
                self.result.add_issue(
                    DataQualityIssue("unsupported_format", f"Unsupported input format: {format_type}", "error")
                )
                return None
            ...
            self.result.add_issue(
                DataQualityIssue("load_error", f"Error loading {format_type} file: {str(e)}", "error")
            )
            return None
    ...
    def _save_data_by_format(self, df: pd.DataFrame, file_path: Path, format_type: str, **kwargs):
        ...
        try:
            if format_type.lower() == 'csv':
                ...
                df.to_excel(file_path, index=False, sheet_name=kwargs.get('sheet_name', 'Sheet1'))
            elif format_type.lower() == 'netcdf':
                try:
                    import xarray as xr
                    ds = df.to_xarray()
                ...
                self.result.add_issue(
                    DataQualityIssue("unsupported_output", f"Unsupported output format: {format_type}", "error")
                )
                return
            ...
            self.result.add_issue(
                DataQualityIssue("save_error", f"Error saving {format_type} file: {str(e)}", "error")
            )