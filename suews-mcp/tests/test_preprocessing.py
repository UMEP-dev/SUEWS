...
class TestForcingDataPreprocessor:
    ...
    def test_load_forcing_data_missing_file(self):
        ...
        assert any("Cannot read file" in issue.message for issue in preprocessor.result.issues)
    ...
    def test_validate_data_structure(self, sample_data_file):
        ...
        structure_issues = [i for i in preprocessor.result.issues if "missing" in i.issue_type]
    ...
    def test_validate_data_structure_missing_columns(self):
        ...
        missing_col_issues = [i for i in preprocessor.result.issues if "missing_columns" in i.issue_type]
    ...
    def test_validate_data_ranges(self, sample_data_file_with_issues):
        ...
        range_issues = [i for i in preprocessor.result.issues if "out_of_range" in i.issue_type]
    ...
    def test_check_missing_data(self, sample_data_file_with_issues):
        ...
        missing_issues = [i for i in preprocessor.result.issues if "missing_data" in i.issue_type]
    ...
    def test_validate_energy_balance(self, sample_data_file):
        ...
        assert "energy_balance_stats" in preprocessor.result.metadata or len(preprocessor.result.issues) > 0
    ...
    def test_preprocess_forcing_file_with_auto_fix(self, sample_data_file_with_issues, tmp_path):
        preprocessor = ForcingDataPreprocessor()
        output_file = tmp_path / "processed_forcing.txt"
...
class TestConfigValidator:
    ...
    def create_sample_config(self, with_issues=False):
        ...
        if with_issues:
            ...
            config["sites"][0]["land_cover"]["paved"]["sfr"]["value"] = 0.8  # Fractions don't sum to 1
    ...
    def test_validate_config_structure_success(self, sample_config_file):
        ...
        structure_issues = [i for i in validator.result.issues if "missing" in i.issue_type and "section" in i.issue_type]
        assert len(structure_issues) == 0
    ...
    def test_validate_config_structure_missing_sections(self):
        ...
        structure_issues = [i for i in validator.result.issues if "missing" in i.issue_type]
        assert len(structure_issues) > 0
    ...
    def test_validate_required_fields(self, sample_config_file):
        ...
        missing_field_issues = [i for i in validator.result.issues if "missing_required_field" in i.issue_type]
    ...
    def test_validate_value_ranges_with_issues(self, sample_config_file_with_issues):
        ...
        invalid_issues = [i for i in validator.result.issues if "invalid" in i.issue_type]
        assert len(invalid_issues) > 0
    
    def test_validate_surface_fractions_invalid_sum(self, sample_config_file_with_issues):
        validator = ConfigValidator()
        config_data = validator._load_config(sample_config_file_with_issues)
        ...
        fraction_issues = [i for i in validator.result.issues if "fraction" in i.issue_type]
...
class TestIntegration:
    ...
    def benchmark_forcing_file(self):
        ...
        benchmark_path = Path(__file__).parent.parent.parent / "test" / "fixtures" / "benchmark1" / "forcing" / "Kc1_2011_data_5.txt"
    ...
    def test_end_to_end_data_workflow(self, tmp_path):
        ...
        assert len(errors) == 0 or all("missing_forcing_file" not in e.issue_type for e in errors)
