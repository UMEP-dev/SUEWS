...
[PASS/FAIL] BENCHMARK GATE (prerequisite, before depositing): candidate build reproducible (byte-identical fingerprint) + per-release schema-valid config + `benchmark/check_regression.py --candidate <version>` passes vs previous release (or regression reviewed and recorded as an accepted entry in benchmark/regression_thresholds.json)
...
4. **Regression decision (the gate)** - run `python benchmark/check_regression.py --candidate <version>` after `assemble_index.py` has added the candidate to `results/index.json`. It compares full-period MAE/|MBE| per flux and RSL air-temperature MAE per height against the previous release under `benchmark/regression_thresholds.json`; a non-zero exit is a **regression: stop and review before depositing**. Only proceed to deposit when the gate passes, or the regression is explicitly reviewed and recorded as an `accepted` entry (with its reason) in the thresholds file so the CI sweep over recorded history keeps passing.
