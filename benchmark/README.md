...
- `bench_regression.py` + `regression_thresholds.json` — the executable regression evaluation: a full-period metric (energy-balance MAE / |MBE| per flux, RSL air-temperature MAE per height) fails when it grows by more than `max(floor, relative * previous)`, and every quantity the previous release carries must be present and finite in the candidate (a candidate with no comparable metrics cannot pass). The tolerance file has a `status`; its numbers are **proposed**, not an approved scientific release policy, and it says how they were derived. Exceptions are narrow: each names one release pair, axis, quantity and metric, a `max_delta` bound, its basis, evidence and reason, and waives nothing else.
- `check_regression.py` — applies that evaluation: `--sweep` is the historical diagnostic over every consecutive release pair in `results/index.json` (CI); `--candidate <version>` evaluates one release against its predecessor. Exit status 1 on a regression or lost coverage; 3 when a candidate is within the proposed tolerances but no approved policy is recorded, so the scientific decision is unavailable and remains a maintainer review.
...
2. **Harness validation + historical diagnostic sweep (active).** Runs the harness unit tests and `check_regression.py --sweep` over the committed `results/index.json`, so the workflow fails if the harness breaks, a recorded release loses metric coverage, or the recorded history and the proposed tolerances in `regression_thresholds.json` fall out of step. No secrets, no model build: it certifies the evaluation mechanism and the recorded history, not the current commit's physics, and it does not make the proposed tolerances an approved policy.
3. **Candidate evaluation (disabled).** Builds the current model, fetches the real restricted observations and forcing via `ZENODO_TOKEN`, runs the benchmark, evaluates it with `check_regression.py --candidate` against the previous release (advisory, exit 3, until a tolerance policy is approved), and publishes the derived statistics on suews.io. Only derived stats are published — no raw data reaches the page, so no page-level password is needed. Enabled once (a) data-governance sign-off is in place to host the observations on a restricted production Zenodo record (with the `ZENODO_TOKEN` secret and `vars.BENCHMARK_PROD_RECID`), and (b) the per-release config and page-wiring are finalised. Until then a green run of this workflow says nothing about the current commit's accuracy; the live comparison of a release candidate is the manual pre-deposit gate in the release process below.

## Reproducibility and the release process

`REPRODUCE.md` recreates every number from scratch. The release process runs this benchmark as a **pre-deposit gate** (see the `prep-release` skill): a release is gated on a reproducible fingerprint and on a maintainer review of the candidate's change versus the previous release, informed by `check_regression.py --candidate <version>`, before it is tagged and published. The tool becomes the decision itself only once `regression_thresholds.json` records an approved policy.
