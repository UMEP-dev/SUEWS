! -----------------------------------------------------------------------------
! SUEWS Rust bridge C API facade for LC_GRASS_PRM.
! -----------------------------------------------------------------------------
module module_c_api_lc_grass_prm
use, intrinsic :: iso_c_binding, only: c_int, c_double, c_char
use module_c_api_common, only: &
   SUEWS_CAPI_OK, SUEWS_CAPI_BAD_BUFFER, SUEWS_CAPI_BAD_STATE, &
   copy_to_c_buffer, suews_capi_error_text
use module_type_landcover, only: LC_GRASS_PRM
...
integer(c_int), parameter, public :: SUEWS_CAPI_LC_GRASS_PRM_LEN = 55_c_int
integer(c_int), parameter, public :: SUEWS_CAPI_LC_GRASS_PRM_SCHEMA_VERSION = 1_c_int
...
public :: suews_lc_grass_prm_len
public :: suews_lc_grass_prm_schema_version
public :: suews_lc_grass_prm_default
public :: suews_lc_grass_prm_error_message
public :: lc_grass_prm_unpack

contains

subroutine suews_lc_grass_prm_len(n_flat, err) bind(C, name='suews_lc_grass_prm_len')
   implicit none

   integer(c_int), intent(out) :: n_flat
   integer(c_int), intent(out) :: err

   n_flat = SUEWS_CAPI_LC_GRASS_PRM_LEN
   err = SUEWS_CAPI_OK

end subroutine suews_lc_grass_prm_len

subroutine suews_lc_grass_prm_schema_version(schema_version, err) bind(C, name='suews_lc_grass_prm_schema_version')
   implicit none

   integer(c_int), intent(out) :: schema_version
   integer(c_int), intent(out) :: err

   schema_version = SUEWS_CAPI_LC_GRASS_PRM_SCHEMA_VERSION
   err = SUEWS_CAPI_OK

end subroutine suews_lc_grass_prm_schema_version

subroutine suews_lc_grass_prm_default(flat, n_flat, err) bind(C, name='suews_lc_grass_prm_default')
   implicit none
   ...
   type(lc_grass_prm_shadow) :: state

   call lc_grass_prm_pack(state, flat, n_flat, err)

end subroutine suews_lc_grass_prm_default

subroutine lc_grass_prm_pack(state, flat, n_flat, err)
   implicit none

   type(lc_grass_prm_shadow), intent(in) :: state
   real(c_double), intent(out) :: flat(*)
   ...
   if (n_flat<SUEWS_CAPI_LC_GRASS_PRM_LEN) then
...
end subroutine lc_grass_prm_pack

subroutine lc_grass_prm_unpack(flat, n_flat, state, err)
   implicit none

   real(c_double), intent(in) :: flat(*)
   integer(c_int), intent(in) :: n_flat
   type(LC_GRASS_PRM), intent(out) :: state
   integer(c_int), intent(out) :: err
...
end subroutine lc_grass_prm_unpack

subroutine suews_lc_grass_prm_error_message(code, buffer, buffer_len) bind(C, name='suews_lc_grass_prm_error_message')
   implicit none
...
end subroutine suews_lc_grass_prm_error_message

end module module_c_api_lc_grass_prm

module c_api_lc_grass_prm_module
use module_c_api_lc_grass_prm
end module c_api_lc_grass_prm_module
