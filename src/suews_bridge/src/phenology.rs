...
crate::codec::impl_state_module_fns! {
    prefix = phenology_state,
    state_type = PhenologyState,
    schema_type = PhenologyStateSchema,
    payload_type = PhenologyStateValuesPayload,
    flat_len_const = PHENOLOGY_STATE_FLAT_LEN,
    schema_version_const = PHENOLOGY_STATE_SCHEMA_VERSION,
    ffi_len_fn = ffi::suews_phenology_state_len,
    ffi_schema_version_fn = ffi::suews_phenology_state_schema_version,
    ffi_default_fn = ffi::suews_phenology_state_default,
}
#[cfg(test)]
mod tests {
    use super::*;
