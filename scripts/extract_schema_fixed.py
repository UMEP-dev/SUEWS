...
def fix_imports_in_file(file_path: Path) -> None:
    ...
    content = re.sub(
        r"from \.\.schema import", "from data_model.schema import", content
    )

    # Replace imports from ..validation with imports from data_model.validation
    content = re.sub(
        r"from \.\.validation import", "from data_model.validation import", content
    )

    # Replace imports from ..yaml_processor with imports from data_model.yaml_processor
    content = re.sub(
        r"from \.\.yaml_processor import",
        "from data_model.yaml_processor import",
        content,
    )
