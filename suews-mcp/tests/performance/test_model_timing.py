...
class TestModelTiming:
    ...
    def performance_forcing_data(self):
        ...
        def generate_forcing(n_hours: int) -> pd.DataFrame:
            ...
            data = {
                ...
                'ldown': 320 + 30 * np.sin(2 * np.pi * (hours % 24) / 24) + np.random.normal(0, 15, n_hours),
            }
    ...
    class PerformanceMetrics:
        ...
        def get_percentile(self, operation: str, percentile: float) -> float:
            ...
            timings = [t['duration'] for t in self.timings if t['operation'] == operation]
            if not timings:
                return 0.0
    ...
    async def test_server_initialization_timing(self, performance_config, perf_metrics):
        ...
        assert stats['mean'] < 0.1, f"Server initialization too slow: {stats['mean']:.3f}s average"
        assert stats['max'] < 0.5, f"Server initialization max time too high: {stats['max']:.3f}s"
        assert stats['std'] < 0.05, f"Server initialization too variable: {stats['std']:.3f}s std"
    ...
    async def test_tool_listing_performance(self, performance_config, perf_metrics):
        ...
        for i in range(n_runs):
            ...
            perf_metrics.record_timing('list_tools', duration, run=i, tool_count=len(result.get('tools', [])))
        
        stats = perf_metrics.get_stats('list_tools')
        
        # Performance assertions
        assert stats['mean'] < 0.01, f"Tool listing too slow: {stats['mean']:.4f}s average"
        assert stats['max'] < 0.05, f"Tool listing max time too high: {stats['max']:.4f}s"
    ...
    async def test_health_check_performance(self, performance_config, perf_metrics):
        ...
        # Performance assertions
        assert stats['mean'] < 0.005, f"Health check too slow: {stats['mean']:.4f}s average"
        assert stats['max'] < 0.02, f"Health check max time too high: {stats['max']:.4f}s"
        ...
        for n_sims in sim_counts:
            sim_timings = [
                t['duration'] for t in perf_metrics.timings 
        ...
        if len(avg_times) >= 2:
            ...
            assert scaling_factor < 3.0, f"Health check scales poorly: {scaling_factor:.2f}x slower with more sims"
    ...
    async def test_configuration_validation_timing(
            ...
            for run in range(n_runs):
                ...
                with patch.object(handlers, '_call_validation_tool') as mock_validate:
                    mock_validate.return_value = {
                        "content": [{"type": "text", "text": f"Config {variant_name} validated"}],
                        "is_error": False
                    }
                    
                    result = await handlers.handle_call_tool("validate_suews_config", validation_args)
                
                end_time = time.perf_counter()
                duration = end_time - start_time
                
                perf_metrics.record_timing(
                    'config_validation', duration,
                    variant=variant_name, sites=n_sites, run=run
                )
        ...
        for variant_name, n_sites in config_variants:
            variant_timings = [
                t['duration'] for t in perf_metrics.timings 
                if t['operation'] == 'config_validation' and t.get('variant') == variant_name
            ]
            
            if variant_timings:
                avg_time = statistics.mean(variant_timings)
                print(f"Config validation ({variant_name}, {n_sites} sites): {avg_time:.4f}s")
                
                # Performance requirements
                if n_sites <= 10:
                    assert avg_time < 0.05, f"Small config validation too slow: {avg_time:.4f}s"
                elif n_sites <= 25:
                    assert avg_time < 0.2, f"Large config validation too slow: {avg_time:.4f}s"

    @pytest.mark.performance
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_simulation_timing_by_duration(
        self, performance_config, performance_forcing_data, benchmark_config_template, 
        perf_metrics, tmp_path
    ):
        ...
        for duration_name, n_hours, n_days in durations:
            ...
            with patch.object(handlers, '_call_simulation_tool') as mock_simulate:
                ...
                await asyncio.sleep(min(simulated_execution_time, 1.0))  # Cap for testing
                ...
                result = await handlers.handle_call_tool("run_suews_simulation", simulation_args)
            ...
            if n_hours <= 168:  # Up to 1 week
                assert duration < 5.0, f"Small simulation {duration_name} too slow: {duration:.3f}s"
            elif n_hours <= 744:  # Up to 1 month
                assert duration < 15.0, f"Medium simulation {duration_name} too slow: {duration:.3f}s"
            else:  # 1 year
                assert duration < 30.0, f"Large simulation {duration_name} too slow: {duration:.3f}s"
        
        # Analyze scaling
        print("\nSimulation timing analysis:")
        for duration_name, n_hours, n_days in durations:
            timing_data = [
                t for t in perf_metrics.timings 
                if t['operation'] == 'simulation' and t.get('duration_name') == duration_name
            ]
    ...
    async def test_concurrent_simulation_performance(
        ...
        async def run_concurrent_simulation(config_file: str, sim_id: str, delay: float = 0):
            ...
            perf_metrics.record_timing(
                'concurrent_simulation', duration,
                sim_id=sim_id, processing_time=processing_time
            )
        ...
        perf_metrics.record_timing('staggered_batch', staggered_total_time, count=n_concurrent)
        ...
        print(f"Concurrent simulations: {concurrent_stats['mean']:.3f}s ± {concurrent_stats['std']:.3f}s")
        print(f"Batch completion: {batch_stats['mean']:.3f}s")
        print(f"Staggered completion: {staggered_stats['mean']:.3f}s")
        
        # Performance assertions
        assert batch_stats['mean'] < 5.0, f"Concurrent batch too slow: {batch_stats['mean']:.3f}s"
        assert concurrent_stats['max'] < 3.0, f"Individual simulation too slow: {concurrent_stats['max']:.3f}s"
    ...
    async def test_memory_usage_scaling(self, performance_config, perf_metrics):
        ...
        for n_simulations in [0, 1, 5, 10, 20, 50]:
            ...
            if n_simulations > 0:
                assert memory_per_sim < 5.0, f"Memory per simulation too high: {memory_per_sim:.2f} MB"
        
        # Analyze memory scaling
        memory_timings = [
            t for t in perf_metrics.timings 
            if t['operation'] == 'memory_usage'
        ]
        ...
        for timing in memory_timings:
            ...
            if n_sims > 0:
                print(f"{n_sims} simulations: +{mem_increase:.1f} MB ({total_mem:.1f} MB total)")
