...
def test_rsl_short_building():
    ...
    try:
        ...
        # Basic sanity checks
        assert np.all(t2_values > -50) and np.all(t2_values < 60), "T2 outside reasonable range"
        assert np.all(rh2_values >= 0) and np.all(rh2_values <= 105), "RH2 outside reasonable range"
        assert np.all(u10_values >= 0) and np.all(u10_values < 100), "U10 outside reasonable range"
...
def test_rsl_very_short_building():
    ...
    df_state_init.loc[:, 'bldgh'] = 2.0  # 2m building height (same as diagnostic height)
    ...
    try:
        ...
        raise AssertionError(f"RSL interpolation failed for very short building: {str(e)}")
