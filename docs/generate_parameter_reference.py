...
def process_unit_part(part: str) -> str:
    ...
    known_patterns = [
        ...
        "s^-1", "kg^-1", "K^-1", "J^-1", "W^-1", "h^-1", "day^-1"
    ]
    ...
    formatted_words = []
    for word in words:
        if word in known_patterns or ("^" in word and any(
...
def extract_parameters_from_model(
    ...
    for field_name, field_info in model_class.model_fields.items():
        ...
        elif hasattr(origin, '__name__') and origin.__name__ in ["RefValue", "FlexibleRefValue"]:
...
def generate_parameter_reference(output_file: Path):
    ...
    rst_content.append("===================")
    rst_content.append("")
    rst_content.append("This page provides a comprehensive, searchable reference of all SUEWS parameters ")
    rst_content.append("available in the YAML configuration format. Parameters are organized alphabetically ")
    ...
    rst_content.append("-----------")
    rst_content.append("")
    rst_content.append("* Use your browser's search function (Ctrl+F / Cmd+F) to find specific parameters")
    rst_content.append("* Search by parameter name (e.g., 'tstep') or full path (e.g., 'model.control.tstep')")
    rst_content.append("* Search by unit (e.g., 'm^2') to find all parameters with that unit")
    rst_content.append("* Search by type (e.g., 'float') to find all parameters of that type")
    rst_content.append("")
...
def generate_parameter_index(parameters: List[Dict], output_file: Path):
    ...
    rst_content.append("===============")
    rst_content.append("")
    rst_content.append("Alphabetical index of all SUEWS parameters. Click on any parameter to jump to its detailed documentation.")
    rst_content.append("")
