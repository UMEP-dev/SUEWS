...
class TestConfigurationFileHandling:
    ...
    async def test_yaml_config_loading_and_validation(self, complete_suews_config, tmp_path):
        ...
        for variant_name, config_data in config_variants.items():
            ...
            assert result.success is True, f"Validation failed for {variant_name} config"
        ...
        print(f"✅ YAML config loading and validation completed for {len(config_variants)} variants")
    ...
    async def test_config_validation_with_references(self, tmp_path):
        ...
        forcing_data = pd.DataFrame({
            'iy': [2024] * 24, 'id': [1] * 24, 'it': list(range(24)), 'imin': [0] * 24,
            'Tair': np.random.normal(15, 3, 24), 'U': np.random.uniform(2, 8, 24),
        ...
        config_with_refs['model']['control']['forcing_file']['value'] = str(tmp_path / 'missing.txt')
    ...
    async def test_config_schema_validation(self, complete_suews_config, tmp_path):
        ...
        missing_section_errors = [i for i in errors if 'missing' in i.issue_type.lower()]
        assert len(missing_section_errors) > 0

    async def test_configuration_backup_and_versioning(self, complete_suews_config, tmp_path):
        ...
        assert current_config['metadata']['version'] != backup_config['metadata']['version']
        assert current_config['model']['control']['tstep'] != backup_config['model']['control']['tstep']
...
class TestFileIOOperations:
    ...
    async def test_concurrent_file_access(self, tmp_path):
        ...
        print(f"✅ Concurrent file access completed: {len(results)} concurrent operations")
...
class TestConfigurationValidationIntegration:
    ...
    async def test_validation_with_mcp_handlers(self, complete_suews_config, tmp_path):
        ...
        mcp_config = MCPServerConfig(
            server_name="validation-integration-test",
            enable_validation_tool=True
        )
        ...
        with patch.object(handlers, '_call_validation_tool') as mock_validation:
            ...
            mock_validation.assert_called_once_with('validate_suews_config', validation_args)
    ...
    async def test_validation_error_reporting_integration(self, tmp_path):
        ...
        assert any(term in combined_messages for term in ['invalid', 'range', 'missing', 'sum'])
