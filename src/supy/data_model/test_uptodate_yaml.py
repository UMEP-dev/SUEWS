...
"""
        ...
        cls.complete_yaml = {
            ...
            'sites': [{'name': 'test', 'gridiv': 1, 'properties': cls.standard_data['sites'][0]['properties'].copy()}]
        }
    ...
    def test_missing_parameter_detection_physics(self):
        ...
        missing_params = find_missing_parameters(self.missing_physics_yaml, self.standard_data)
        
        # Filter for physics parameters
        physics_missing = [(path, val, is_physics) for path, val, is_physics in missing_params if is_physics]
        ...
        netradiationmethod_entry = next((item for item in missing_params if item[0] == 'model.physics.netradiationmethod'), None)
        self.assertIsNotNone(netradiationmethod_entry)
        self.assertTrue(netradiationmethod_entry[2], "netradiationmethod should be marked as physics/URGENT")

    def test_missing_parameter_detection_optional(self):
        """Test detection of missing optional parameters.""" 
        missing_params = find_missing_parameters(self.missing_optional_yaml, self.standard_data)
        
        # Filter for non-physics parameters
        optional_missing = [(path, val, is_physics) for path, val, is_physics in missing_params if not is_physics]
        
        # Should find many missing optional parameters
        self.assertGreater(len(optional_missing), 0, "Should find missing optional parameters")
        
        # All should be marked as non-physics (optional)
        for _, _, is_physics in optional_missing:
            self.assertFalse(is_physics, "Optional parameters should not be marked as physics")

    def test_renamed_parameter_handling(self):
        """Test handling of renamed parameters."""
        updated_content, replacements = handle_renamed_parameters(self.renamed_yaml_content)
    ...
    def test_extra_parameter_detection(self):
        ...
        standard_paths = [path for path in extra_params if 'tstep' in path or 'netradiationmethod' in path]
        self.assertEqual(len(standard_paths), 0, "Standard parameters should not be marked as extra")
    ...
    def test_no_missing_parameters_scenario(self):
        ...
        self.assertEqual(len(missing), 0, "Should find no missing parameters in complete match")
    ...
    def test_renamed_params_consistency(self):
        ...
        for old_name, new_name in RENAMED_PARAMS.items():
            self.assertNotEqual(old_name, new_name, f"Renamed parameter {old_name} -> {new_name} should be different")
    ...
    def test_physics_options_completeness(self):
        ...
        for option in expected_physics_options:
            self.assertIn(option, PHYSICS_OPTIONS, f"Physics option {option} should be in PHYSICS_OPTIONS")
        
        # All entries should be strings
        for option in PHYSICS_OPTIONS:
            self.assertIsInstance(option, str, f"Physics option {option} should be a string")
...
class TestRealWorldScenarios(unittest.TestCase):
    ...
    def test_benchmark_configuration_compatibility(self):
        ...
        urgent_params = [(path, val, is_physics) for path, val, is_physics in missing_params if is_physics]
        urgent_paths = [path for path, _, _ in urgent_params]
        
        self.assertIn('model.physics.netradiationmethod', urgent_paths, 
                     "Should detect missing netradiationmethod as URGENT")
        
        # Should find many optional parameters 
        optional_params = [(path, val, is_physics) for path, val, is_physics in missing_params if not is_physics]
        self.assertGreater(len(optional_params), 0, "Should find missing optional parameters")
    ...
    def test_standard_config_integrity(self):
        ...
        for physics_param in ['netradiationmethod', 'emissionsmethod', 'storageheatmethod']:
            self.assertIn(physics_param, physics, f"Standard config should contain {physics_param}")
...
"""
            ...
            self.assertTrue(os.path.exists(uptodate_file), "Uptodate YAML file should be created")
            ...
            self.assertNotIn('diagmethod:', uptodate_content, "Old parameter name should be replaced")
            self.assertNotIn('cp:', uptodate_content, "Old parameter name should be replaced")
            self.assertIn('rslmethod:', uptodate_content, "New parameter name should be present")
            self.assertIn('rho_cp:', uptodate_content, "New parameter name should be present")
            
            # Should have MISSING parameters added with null values
            self.assertIn('netradiationmethod:', uptodate_content, "Missing URGENT parameter should be added")
            uptodate_yaml_data = yaml.safe_load(uptodate_content)
            self.assertEqual(uptodate_yaml_data['model']['physics']['netradiationmethod']['value'], 
                           None, "Missing parameter should have null value")
            
            # Should contain NOT IN STANDARD parameters (preserved)
            self.assertIn('custom_control_param:', uptodate_content, "NOT IN STANDARD params should be preserved")
            self.assertIn('custom_physics_param:', uptodate_content, "NOT IN STANDARD params should be preserved")
            self.assertIn('custom_site_property:', uptodate_content, "NOT IN STANDARD params should be preserved")
            
            # Should NOT contain inline comments in clean YAML
            self.assertNotIn('#RENAMED IN STANDARD', uptodate_content, "Clean YAML should have no inline comments")
            self.assertNotIn('#NOT IN STANDARD', uptodate_content, "Clean YAML should have no inline comments")
            self.assertNotIn('#MISSING', uptodate_content, "Clean YAML should have no inline comments")
            ...
            self.assertIn('URGENT', report_content, "Should identify URGENT physics parameters")
            self.assertIn('netradiationmethod', report_content, "Should list missing physics parameter")
            self.assertIn('physics options', report_content, "Should categorize physics options")
            
            # Should list renamed parameters with old -> new mapping
            self.assertIn('diagmethod -> rslmethod', report_content, "Should show parameter renaming")
            self.assertIn('cp -> rho_cp', report_content, "Should show parameter renaming")
            
            # Should list NOT IN STANDARD parameters
            self.assertIn('custom_control_param', report_content, "Should list extra control parameter")
            self.assertIn('custom_physics_param', report_content, "Should list extra physics parameter")
            self.assertIn('custom_site_property', report_content, "Should list extra site parameter")
            
            # Should contain usage instructions with manual link
            self.assertIn('https://suews.readthedocs.io/latest/', report_content, "Should contain manual link")
            self.assertIn('uptodate_user.yml', report_content, "Should reference output file")
            ...
            self.assertEqual(updated_data['model']['control']['tstep'], original_data['model']['control']['tstep'])
            
            # NOT IN STANDARD parameters should be preserved
            self.assertEqual(updated_data['model']['control']['custom_control_param'], 
                           original_data['model']['control']['custom_control_param'])
            self.assertEqual(updated_data['model']['physics']['custom_physics_param'], 
                           original_data['model']['physics']['custom_physics_param'])
            
            # RENAMED parameters should be updated
            self.assertNotIn('diagmethod', updated_data['model']['physics'], "Old parameter should be removed")
            self.assertNotIn('cp', updated_data['model']['physics'], "Old parameter should be removed")
            self.assertIn('rslmethod', updated_data['model']['physics'], "New parameter should be added")
            self.assertIn('rho_cp', updated_data['model']['physics'], "New parameter should be added")
            ...
            self.assertIn('netradiationmethod', updated_data['model']['physics'], "Missing parameter should be added")
            self.assertIsNone(updated_data['model']['physics']['netradiationmethod']['value'], 
            ...
            self.assertIn('MISSING IN STANDARD parameters', summary_text, "Should count missing parameters")
            self.assertIn('RENAMED IN STANDARD parameters', summary_text, "Should count renamed parameters")  
            self.assertIn('NOT IN STANDARD parameters', summary_text, "Should count extra parameters")
            ...
            self.assertIn('control', updated_data['model'], "Should maintain control section")
            self.assertIn('physics', updated_data['model'], "Should maintain physics section")
            ...
            self.assertGreater(len(updated_data['sites']), 0, "Should have at least one site")
            
            print(f"\n✅ End-to-end test completed successfully!")
            print(f"   - Generated uptodate YAML: {os.path.getsize(uptodate_file)} bytes")
    ...
    def test_workflow_performance_and_scalability(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(10):  # 10 sites to test scalability
                site = {
                    ...
                    'properties': {
                        ...
                        f'custom_param_{i}': {'value': f'custom_value_{i}'}  # NOT IN STANDARD
                    }
                }
            ...
            for i in range(10):
                ...
                self.assertIn(f'custom_param_{i}', uptodate_content, f"Should preserve custom_param_{i}")
            ...
            self.assertLess(processing_time, 10.0, "Processing should complete within reasonable time")
