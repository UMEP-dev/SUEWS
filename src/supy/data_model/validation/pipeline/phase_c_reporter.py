...
def generate_phase_c_text_report(json_data: Dict[str, Any], phase: str, mode: str) -> str:
    """
    ...
    if phase_c_errors:
        ...
        lines.append(f"- Found ({len(phase_c_errors)}) critical Pydantic validation error(s):")
    ...
    if len(phase) > 1:
        ...
        if renamed:
            no_action_items.append(f"- Updated ({len(renamed)}) renamed parameter(s) to current standards:")
            for item in renamed:
        ...
        optional = [i for i in phase_a_info if i.get("type") == "missing_optional_parameter"]
        if optional:
            no_action_items.append(f"- Updated ({len(optional)}) optional missing parameter(s) with null values:")
            for item in optional:
                param_name = item.get("details", {}).get("parameter_name", "")
                no_action_items.append(f"-- {param_name} added to the updated YAML and set to null")
        
        # Phase B scientific warnings
        if phase_b_warnings:
            no_action_items.append(f"- Found ({len(phase_b_warnings)}) scientific warning(s) for information:")
...
def generate_phase_c_report_refactored(
    ...
    text_report = generate_phase_c_text_report(reporter.get_json_report(), phase_str, mode)
