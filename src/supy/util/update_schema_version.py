    ...
    try:
        ...
        if 'version' in config or 'config_version' in config:
            ...
            if 'description' in config:
                ...
                desc = re.sub(r'Sample config v[\d\.]+ designed for supy version [\d\.]+(?:\.dev\d+)?, ', '', desc)
...
def find_yaml_configs(root_path: Path, patterns: Optional[List[str]] = None) -> List[Path]:
    """
    ...
    excluded_dirs = {'.github', '.gitlab', 'ci', '.circleci', 'node_modules', '.venv', '__pycache__'}
    config_files = []
    for f in files:
        if not any(excluded in f.parts for excluded in excluded_dirs):
            ...
            try:
                with open(f, 'r') as file:
                    ...
                    if isinstance(content, dict) and ('model' in content or 'sites' in content):
...
Examples:
  ...
  %(prog)s old_config.yml --schema-version 1.0
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=Path,
        help="YAML configuration files to update"
    )
    parser.add_argument(
        "--directory", "-d",
        type=Path,
