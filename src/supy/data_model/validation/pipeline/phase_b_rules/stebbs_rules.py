...
def check_archetype_properties(context):
    ...
    Notes
    -----
    - The rule only applies if the "stebbs" in the physics model is set to 1.
...
def check_daylight_control(context):
    ...
    - Checks that the 'DaylightControl' flag under each site's 'stebbs' properties is set to 0 or 1
      (accepting both integer and float representations), but only if the 'stebbs' in the model
      physics is set to 1.
