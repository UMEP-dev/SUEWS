...
from .thermal_comfort_analyzer import calculate_humidex, calculate_wbgt_simple, categorize_thermal_stress
...
def analyze_simulation_context(config: Dict, research_context: str) -> Dict[str, Any]:
    ...
    if 'SiteCharacteristics' in config:
        ...
        context["site_characteristics"] = {
            ...
            "vegetation_fraction": site.get('fr_evertr', 0) + site.get('fr_dectr', 0) + site.get('fr_grass', 0),
...
def generate_interpretation_section(
    ...
    if any(word in research_lower for word in ['energy', 'efficiency', 'consumption']):
        ...
        if 'QF' in df.columns:
            qf_mean = df['QF'].mean()
            qf_summer = df[df.index.month.isin([6,7,8])]['QF'].mean() if len(df) > 365*24 else qf_mean
    ...
    try:
        ...
        if output_p.suffix == '.csv':
            ...
            df['datetime'] += pd.to_timedelta(df['Hour'], unit='h') + \
                             pd.to_timedelta(df.get('Min', 0), unit='m')
