...
async def call_tool(tool_name, args):
    ...
    server_params = StdioServerParameters(
        command="python", args=["run_server.py"], env={"PYTHONPATH": "src"}
    )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            '  python mcp_cli.py check-physics \'{"NetRadiationMethod":"LDOWN_AIR","EmissionsMethod":"J19"}\''
        )
        sys.exit(1)
    ...
    if command == "list":
        ...
        result = asyncio.run(
            call_tool(
                "explain_suews_parameter", {"parameter_name": param, "include_examples": True}
            )
        )
        print(result)
        ...
        result = asyncio.run(
            call_tool("validate_suews_config", {"config_path": config_path, "strict": False})
        )
        print(result)
        ...
        result = asyncio.run(
            call_tool(
                ...
                {"site_type": site_type, "research_focus": focus, "include_comments": True},
            )
        )
        print(result)
        ...
        result = asyncio.run(call_tool("check_suews_physics_compatibility", {"methods": methods}))
        print(result)
