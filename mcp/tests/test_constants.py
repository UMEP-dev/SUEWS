...
def test_allowed_subcommands_contain_phase1_set() -> None:
    from suews_mcp.constants import ALLOWED_SUBCOMMANDS

    for cmd in ("validate", "schema", "convert", "run", "inspect", "diagnose", "summarise", "compare", "init"):
...
def test_uri_prefixes_share_scheme() -> None:
    ...
    for prefix in (URI_SCHEMA_PREFIX, URI_EXAMPLES_PREFIX, URI_DOCS_PREFIX, URI_RUNS_PREFIX):
