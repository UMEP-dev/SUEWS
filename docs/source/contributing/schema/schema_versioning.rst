...
**Schema 2026.6.dev2** (current; development)
   Development schema adding ``model.control.output.timestamp_reference`` for
   presentation-only output timestamp relabelling. Existing
   ``2026.6.dev1`` YAMLs remain valid because the new output option
   defaults to ``follow``; the ``(2026.6.dev1 -> 2026.6.dev2)``
   handler is a no-op content migration that updates the schema marker.
