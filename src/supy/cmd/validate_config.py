...
def validate_single_file(
    ...
    try:
        ...
        if check_migration_needed(str(file_path)):
            errors.append(f"Configuration uses old schema version and may need migration")
...
def check(file):
    ...
    try:
        ...
        if schema_version == 'not specified':
            console.print("[yellow]  ⚠ No schema version specified, assuming v{CURRENT_SCHEMA_VERSION}[/yellow]")
        elif schema_version != CURRENT_SCHEMA_VERSION:
            console.print(f"[yellow]  ⚠ Different from current ({CURRENT_SCHEMA_VERSION})[/yellow]")
        ...
        console.print(f"  • Model timestep: {config.get('model', {}).get('control', {}).get('tstep', 'not set')}s")
...
def migrate(file, output, to_version):
    ...
    try:
        ...
        if current_version == target_version:
            console.print("[yellow]Already at target version, no migration needed[/yellow]")
            return
        
        # Migrate
        console.print(f"Migrating to: {target_version}")
        migrated = migrator.migrate(config, from_version=current_version, to_version=target_version)
        ...
        if is_valid:
            ...
            console.print("[yellow]⚠ Migrated configuration may need manual adjustments[/yellow]")
