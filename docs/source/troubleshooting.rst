...
Check that your YAML configuration follows the current schema and compare it
with the configuration returned by
``SUEWSSimulation.from_sample_data().config``. For continuation runs, use a
:class:`~supy.SUEWSCheckpoint` created by the previous output rather than a
manually edited state DataFrame.
