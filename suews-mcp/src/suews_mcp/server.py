...
class SUEWSMCPServer:
    ...
    def __init__(self, config: Optional[MCPServerConfig] = None):
        ...
        if not MCP_AVAILABLE or not HANDLERS_MCP_AVAILABLE:
            logger.warning("MCP library not available - server will run in test mode only")
            self.server = None
        else:
        ...
        logger.info(f"SUEWS MCP Server initialized: {self.config.server_name} v{self.config.server_version}")
    ...
    def _register_handlers(self):
        ...
        async def handle_initialize(params):
            return await self.handlers.handle_initialize(params.dict() if hasattr(params, 'dict') else params)
    ...
    async def run(self):
        ...
        if not MCP_AVAILABLE or self.server is None:
            ...
            raise RuntimeError("MCP library not available - please install the 'mcp' package")
