...
try:
    from supy.data_model.core.site import ArchetypeProperties
...
AR_CH_FIELDS = ["HeatingSetpointTemperature", "CoolingSetpointTemperature", "OccupantsProfile"]
DF_BASE_NAMES = ["heatingsetpointtemperature", "coolingsetpointtemperature", "occupantsprofile"]
...
def get_archetype_properties(site_doc):
    """
    Extract the dict that can be used to construct an ArchetypeProperties
    instance from the site document parsed from YAML.
    """
...
def main():
    ...
    try:
        archetype = ArchetypeProperties(**archetype_dict)
    except Exception as e:
        print("Validation/Construction error for ArchetypeProperties:", e)
        raise

    print("\nConstructed ArchetypeProperties (partial dump):")
    try:
    ...
    for field_name in AR_CH_FIELDS:
        ...
        print(f"\nFound ArchetypeProperties.{field_name} attribute:", value is not None)

    try:
        df = archetype.to_df_state(GRID_ID)
    except Exception as e:
        print("archetype.to_df_state failed:", e)
        raise
