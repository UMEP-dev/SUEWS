...
class TestMCPIntegration:
    ...
    async def test_full_workflow_with_sample_data(self):
        ...
        with patch('supy.mcp.tools.configure.SUEWSConfig', mock_config), \
            ...
            configure_result = await self.server.call_tool("configure_simulation", {
                "validate_only": False
            })
        ...
        with patch('supy.mcp.tools.run.run_supy', return_value=mock_output), \
             patch('supy.mcp.tools.run.load_SampleData', return_value=(mock_state, mock_forcing)):
            
            run_result = await self.server.call_tool("run_simulation", {
    ...
    async def test_error_handling_chain(self):
        ...
        run_result = await self.server.call_tool("run_simulation", {
            "forcing_path": "/nonexistent/forcing.txt"
        })
        ...
        analyze_result = await self.server.call_tool("analyze_results", {
            "results_path": "/nonexistent/results.csv"
        })
    ...
    async def test_parameter_validation_across_tools(self):
        ...
        assert analyze_result["success"] is False
        assert any("Missing required parameters" in error for error in analyze_result.get("errors", []))
    ...
    async def test_concurrent_tool_execution(self):
        ...
        try:
            ...
            tasks = [
                self.server.call_tool("analyze_results", {
...
class TestMCPErrorScenarios:
    ...
    async def test_large_data_handling(self):
        ...
        try:
            result = await self.server.call_tool("analyze_results", {
                "results_path": temp_path,
                "analysis_type": "summary"
            })
