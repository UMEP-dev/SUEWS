...
class ParameterTranslator:
        ...
        if create and not path_obj.exists():
            try:
                ...
                raise PermissionError(
                    f"Permission denied to create directory: {path_obj}"
                )
            except Exception as e:
    ...
    def validate_time_range(
        start_time: Optional[str], end_time: Optional[str], allow_same: bool = False
    ) -> tuple:
        """
    ...
    def validate_analysis_variables(
        ...
        for var in var_list:
            if not isinstance(var, str):
                raise ValueError(
                    f"Variable name must be string, got {type(var)}: {var}"
                )
    ...
    def serialize_dataframe(
        df: pd.DataFrame, max_rows: int = 100, include_stats: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if df is None or df.empty:
            return {"shape": [0, 0], "columns": [], "data": [], "empty": True}
