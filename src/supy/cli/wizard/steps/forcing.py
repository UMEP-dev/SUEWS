...
class ForcingDataStep(WizardStep):
    ...
    def collect_input(self) -> Dict[str, Any]:
        ...
        console.print(
            "[dim]Provide the path to your meteorological forcing data file[/dim]"
        )

        default_path = self.session.get_value("forcing.file_path", "")
        while True:
            file_path = Prompt.ask(
                "Forcing data file path", default=default_path or "./forcing_data.txt"
            )
        ...
        data["forcing.resolution"] = IntPrompt.ask(
            "Forcing data time resolution [seconds]", default=default_res
        )
        ...
        if data["forcing.resolution"] > timestep:
            console.print(
                f"[yellow]Warning: Forcing resolution ({data['forcing.resolution']}s) is larger than timestep ({timestep}s)[/yellow]"
            )
        ...
        if not use_standard:
            ...
            console.print(
                "[dim]Please ensure your file uses standard SUEWS variable names[/dim]"
            )
        ...
        use_multiple = Confirm.ask(
            "Do you have site-specific forcing files?", default=False
        )
        data["forcing.multiple_files"] = use_multiple

        if use_multiple:
            console.print(
                "[yellow]Multiple forcing files setup not yet implemented[/yellow]"
            )
    ...
    def validate(self, data: Dict[str, Any]) -> bool:
        ...
        if resolution <= 0:
            self.session.add_validation_error(
                "resolution", "Forcing data resolution must be positive"
            )
            valid = False
        elif resolution < 60:
            console.print(
                "[yellow]Warning: Very high resolution data may impact performance[/yellow]"
            )
