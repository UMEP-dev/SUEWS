...
class ConfigurationValidator:
    ...
    def _check_surface_fractions(self, config: Dict) -> Dict:
        ...
        fractions = []
        fraction_names = ['fr_paved', 'fr_bldgs', 'fr_evetr', 'fr_dectr', 
        ...
        if start and end:
            try:
                ...
                if (end_dt - start_dt).days > 365:
                    issues.append("Simulation period > 1 year (may use excessive memory)")
            except:
...
class ForcingDataValidator:
    ...
    def __init__(self):
        self.required_columns = [
            ...
            'qn1', 'qh', 'qe', 'qs', 'qf',  # Energy fluxes
            'U', 'RH', 'Tair', 'pres',  # Meteorological
            'rain', 'kdown', 'snow', 'ldown',  # Precipitation and radiation
            'fcld', 'Wuh', 'xsmd', 'lai',  # Additional variables
            'kdiff', 'kdir', 'wdir'  # Radiation components
        ]
        self.error_handler = SUEWSErrorHandler()
    ...
    def validate(self, data_path: Path) -> ValidationResult:
        ...
        try:
            ...
            if not missing_cols:
                ...
                suggestions.add("Ensure all required meteorological variables are present")
            ...
            if passed:
                ...
                summary = f"Forcing data has {critical_count} critical issues to resolve."
    ...
    def _check_data_completeness(self, df: pd.DataFrame) -> Dict:
        ...
        critical_missing = {col: pct for col, pct in missing_pct.items() 
