...
fn main() {
    ...
    // Lint-only mode (e.g. `cargo clippy` in CI): skip all native Fortran
    // compilation and linking. clippy only type-checks the crate, so the
    // Fortran object files and prebuilt static libs are never needed -- they
    // matter solely at link time, which a check build never reaches. The one
    // build-script output the Rust sources consume at check time is
    // `output_cols.rs`, and that is generated by parsing Fortran *source*
...
fn generate_output_col_constants(manifest_dir: &Path, out_dir: &Path) {
...
fn link_fortran_runtime(gfortran_bin: &Path) {
    let target_os = env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
