...
def test_timezone_enum_support():
    ...
    try:
        ...
        site = config.sites[0]
        timezone_value = site.properties.timezone.value if hasattr(site.properties.timezone, 'value') else site.properties.timezone
        assert isinstance(timezone_value, TimezoneOffset), f"Expected TimezoneOffset enum, got {type(timezone_value)}"
        assert timezone_value == TimezoneOffset.UTC_PLUS_5_30, f"Expected UTC_PLUS_5_30, got {timezone_value}"
        assert timezone_value.value == 5.5, f"Expected value 5.5, got {timezone_value.value}"
...
def test_timezone_boundary_values():
    ...
    for tz_value in test_values:
        ...
        try:
            ...
            site = config.sites[0]
            timezone_value_loaded = site.properties.timezone.value if hasattr(site.properties.timezone, 'value') else site.properties.timezone
            assert isinstance(timezone_value_loaded, TimezoneOffset), f"Expected TimezoneOffset enum, got {type(timezone_value_loaded)}"
            assert timezone_value_loaded.value == tz_value, f"Expected timezone {tz_value}, got {timezone_value_loaded.value}"
...
def test_timezone_validation_errors():
    ...
    invalid_values = [-13.0, 13.0, -12.1, 12.1, 5.25, 7.75, -0.5]  # Not standard timezone offsets
    ...
    for tz_value in invalid_values:
        ...
        try:
            ...
            with pytest.raises(Exception):  # Could be ValidationError or other pydantic error
