...
def analyze_diurnal_patterns(df: pd.DataFrame, variable: str) -> Dict[str, float]:
    ...
    return {
        ...
        "nighttime_mean": float(df[~df.index.to_series().between('06:00', '18:00')][variable].mean()),
    }
    ...
    for col in surf_temp_cols:
        if col in df.columns:
            patterns[col] = {
                ...
                "diurnal_range": float(df.groupby(df.index.date)[col].apply(lambda x: x.max() - x.min()).mean()),
...
def analyze_energy_partitioning(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if 'QN' in df.columns and 'QH' in df.columns and 'QE' in df.columns:
        ...
        if len(daytime_positive) > 0:
            results['bowen_ratio'] = float((daytime_positive['QH'] / daytime_positive['QE']).replace([np.inf, -np.inf], np.nan).mean())
            results['evaporative_fraction'] = float((daytime_positive['QE'] / daytime_positive['QN']).mean())
            results['sensible_fraction'] = float((daytime_positive['QH'] / daytime_positive['QN']).mean())
            
            if 'QS' in df.columns:
                results['storage_fraction'] = float((daytime_positive['QS'] / daytime_positive['QN']).mean())
...
def identify_urban_cooling_potential(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if 'Kup' in df.columns and 'Kdown' in df.columns:
        ...
        potential['albedo_increase_cooling'] = float(0.1 * df['Kdown'].mean() * 0.5)  # Rough estimate
...
def calculate_urban_modification_factors(urban_df: pd.DataFrame, rural_df: Optional[pd.DataFrame] = None) -> Dict[str, float]:
    ...
    if rural_df is not None and len(urban_df) == len(rural_df):
        ...
        if 'T2' in urban_df.columns and 'T2' in rural_df.columns:
            factors['temperature_amplification'] = float(
                (urban_df['T2'] - rural_df['T2']).mean()
            )
            factors['max_uhi_intensity'] = float(
                (urban_df['T2'] - rural_df['T2']).max()
            )
        ...
        if 'RH2' in urban_df.columns and 'RH2' in rural_df.columns:
            factors['humidity_reduction'] = float(
                (rural_df['RH2'] - urban_df['RH2']).mean()
            )
        
        # Wind modification
        if 'U10' in urban_df.columns and 'U10' in rural_df.columns:
            factors['wind_reduction_factor'] = float(
                1 - (urban_df['U10'] / rural_df['U10']).mean()
            )
    ...
    try:
        ...
        if urban_path.suffix == '.csv':
            ...
            urban_df['datetime'] = pd.to_datetime(
                urban_df['Year'].astype(str) + urban_df['DOY'].astype(str), 
                format='%Y%j'
            )
            urban_df['datetime'] += pd.to_timedelta(urban_df['Hour'], unit='h') + \
                                   pd.to_timedelta(urban_df.get('Min', 0), unit='m')
        ...
        if rural_reference:
            ...
            if rural_path.exists():
                if rural_path.suffix == '.csv':
                    rural_df = pd.read_csv(rural_path, parse_dates=['datetime'], index_col='datetime')
                elif rural_path.suffix == '.txt':
                    rural_df = pd.read_csv(rural_path, sep=r'\s+')
                    rural_df['datetime'] = pd.to_datetime(
                        rural_df['Year'].astype(str) + rural_df['DOY'].astype(str), 
                        format='%Y%j'
                    )
                    rural_df['datetime'] += pd.to_timedelta(rural_df['Hour'], unit='h') + \
        ...
        if energy_partition:
            ...
            if 'evaporative_fraction' in energy_partition:
                output += f"- Evaporative fraction: {energy_partition['evaporative_fraction']:.2%}\n"
        ...
        if rural_df is not None:
            ...
            if mod_factors:
                ...
                if 'wind_reduction_factor' in mod_factors:
                    output += f"- Wind speed reduction: {mod_factors['wind_reduction_factor']:.0%}\n"
