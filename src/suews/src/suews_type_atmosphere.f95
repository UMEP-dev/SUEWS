module module_type_atmosphere
   ...
   TYPE, PUBLIC :: atm_state
      REAL(KIND(1D0)) :: f_cloud = 0.0D0 !estomated cloud fraction [-]
      REAL(KIND(1D0)) :: av_cp = 0.0D0 !Specific heat capacity
      REAL(KIND(1D0)) :: dens_dry = 0.0D0 !Dry air density kg m-3
      REAL(KIND(1D0)) :: av_density = 0.0D0 !Average air density
      ...
      REAL(KIND(1D0)) :: lv_s_j_kg = 0.0D0 !latent heat of sublimation [J kg-1]
      ...
      REAL(KIND(1D0)) :: psyc_ice_h_pa = 0.0D0 !Psychometric constant in hPa for snow
      ...
      REAL(KIND(1D0)) :: s_ice_hpa = 0.0D0 !Vapour pressure versus temperature slope in hPa above ice/snow
      ...
      REAL(KIND(1D0)) :: z_l = 0.0D0 ! Stability scale [-]
      REAL(KIND(1D0)) :: RA_h = 0.0D0 ! aerodynamic resistance [s m-1]
      REAL(KIND(1D0)) :: RS = 0.0D0 ! surface resistance [s m-1]
      REAL(KIND(1D0)) :: u_star = 0.0D0 !friction velocity [m s-1]
      REAL(KIND(1D0)) :: t_star = 0.0D0 !T*, temperature scale [-]
