...
def build_phase_b_reporter(
    ...
    for result in validation_results:
        if result.status == "ERROR":
            ...
            reporter.add_error({
                ...
                "suggestions": [result.suggested_value]
    ...
    for adjustment in adjustments:
        ...
        if (
            "temperature" in adjustment.parameter
            or "tsfc" in adjustment.parameter
            or "tin" in adjustment.parameter
        ):
...
def generate_phase_b_text_report(
    ...
    if phase_b_errors:
        ...
        lines.append(
            f"- Found ({len(phase_b_errors)}) critical scientific parameter error(s):"
        )
    ...
    adjustments = [
        i for i in phase_b_info if i.get("details", {}).get("auto_corrected")
    ]
    ...
    if adjustments:
        ...
        for adj in adjustments:
            ...
            lines.append(
                f"-- {field_path}{site_ref}: {old_value} → {new_value} ({reason})"
            )
...
def run_science_check_refactored(
    ...
    try:
        ...
        reporter = build_phase_b_reporter(validation_results, adjustments, mode)
    ...
    if science_report_file:
        ...
        text_report = generate_phase_b_text_report(
            reporter.get_json_report(), phase, mode
        )
